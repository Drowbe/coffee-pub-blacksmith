...
| `node tools/check-quick-rolls.mjs` | the Quick Rolls library: every `data-*` a row's readers want is one the row writer sets, `data-dc` stays conditional, and the built-in rolls are all still there. A world seeds from them exactly once, so a lost default is lost silently and forever |
