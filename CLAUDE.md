...
| `node tools/check-coffeepub-constants.mjs` | every `COFFEEPUB.NAME` a script reads is defined by some config or theme, and every numeric fallback beside one agrees with the value it replaces. A missing constant is `undefined`, which a default parameter quietly turns into a number -- nothing throws and nothing logs, and the only symptom is a wrong value. `SOUNDVOLUMELOW` never existed and played a cinematic sound at 0.7 where 0.5 was meant |
...
| `node tools/check-legacy-surfaces.mjs` | the two legacy-surface tables and our own conformance: `HookManager.LEGACY_HOOKS` and `LEGACY_GLOBALS` stay well-formed, the console commands they advertise are wired, and no file registers a retired hook name that is a plain rename. Registering a retired hook **succeeds** and never fires, so nothing else catches it |
