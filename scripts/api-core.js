...
export function setupCollapsibleCards() {
    ...
    if (chatLog && !chatLog.hasAttribute('data-blacksmith-collapsible-handler')) {
        // Add event delegation handler - use capture phase to catch events early
...
export function attachCollapsibleHandlerToCard(cardElement) {
    ...
    const header = cardElement.querySelector('.card-header.collapsible');
    if (!header) {
        return;
    }
...
/**
 * Handle clicks on collapsible card headers
 * Toggles the collapsed state of the section-content within the same card
 */
function handleCollapsibleCardClick(event) {
    ...
    // Get the data-section attribute from the header
    const sectionId = header.getAttribute('data-section');
    if (!sectionId) {
        console.error('Blacksmith: Header missing data-section attribute', header);
        return;
    }
    
    // Find the card container (blacksmith-card) to scope our search
    const card = header.closest('.blacksmith-card');
    if (!card) {
        console.error('Blacksmith: Could not find .blacksmith-card parent', header);
        return;
    }
    
    // Find the section-content with matching data-section attribute
    const content = card.querySelector(`.section-content[data-section="${sectionId}"]`);
    if (!content) {
        console.error('Blacksmith: Could not find .section-content with data-section', { 
            sectionId, 
            card, 
            availableSections: Array.from(card.querySelectorAll('.section-content')).map(el => el.getAttribute('data-section'))
        });
        return;
    }
    ...
    if (isCollapsed) {
        ...
        // Collapse - add collapsed class to section-content
