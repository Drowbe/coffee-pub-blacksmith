...
- **Motion on the encounter bar** (`scripts/api-menubar.js`, `styles/menubar-widgets.css`, `scripts/manager-combatbar.js`). Four behaviours, all driven by a real value change and never by a render:
  ...
  - **A burst when a record falls** - the one animation allowed to be loud, because it marks the rarest thing the bar reports, and rarity is what stops it becoming wallpaper. A ring expanding out of the chip rather than the chip growing, so nothing on the row moves.
- **`burst: true` is a caller's signal, not an inference.** The menubar can see a number rise; it cannot see that the rise passed a campaign best, because the standing record lives in the lifetime tier while the value pushed is from the running fight. `manager-combatbar.js` compares the two and latches on the amount already celebrated - the same swing is pushed on every refresh for as long as it stands, so an unlatched "live beats record" test would burst several times a second for the rest of the fight. The latch clears on `blacksmith.combatSummaryReady`, which is when the standing record actually moves.
- **All of it is opt-out under `prefers-reduced-motion`, and the opt-out is a plain swap rather than a shorter animation** - someone who asked for less motion did not ask for faster motion. The preference is read at the moment of animating rather than cached, so changing it mid-session takes effect immediately.
...
- **Verify live**: land a hit and confirm the damage chip counts up and flashes green; take damage and confirm the damage-taken chip flashes red; beat the campaign's biggest hit and confirm exactly one burst rather than one per refresh; watch a standing change hands and confirm the portrait fades rather than cutting. With reduced motion on, confirm every value simply swaps.
