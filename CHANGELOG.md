...
- **Verify live**: with the field empty, an excluded user sees every channelled toast. Open Settings and confirm each declared channel appears as a ticked row; untick one and confirm the field fills in with the rest; re-tick everything and confirm it clears to empty. Type a name no module registers, reopen the form, and confirm it is still listed, still ticked, and marked as not currently registered.
