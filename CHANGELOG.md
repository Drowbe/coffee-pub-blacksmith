...
- **A named animation vocabulary shared by every part** (`styles/cards-parts.css`). `shake-x`, `shake-y`, `pulse`, `glow`, applied as `.cpb-anim-{name}`, suppressed under `prefers-reduced-motion`. A card names a **motion, never a meaning**: a critical hit is `tone: 'success'` with `animation: 'shake-y'`, but the part does not know that pairing, so the same motion is available to a crafting success or a countdown. The existing crit and fumble rules bound the two together, which put a d20 rule inside the presentation layer. The shake keyframes moved here from `cards-skill-check.css`, which later steps delete -- keyframes are document-global, so a definition sitting in a doomed file would have taken the animation down with it silently.
