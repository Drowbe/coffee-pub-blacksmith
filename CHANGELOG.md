...
- **`testing/preview-chat-cards.js`** -- paste into a script macro to post every part, every variant, and one card per theme. Most card defects are things only a person can see -- alignment, spacing, whether two treatments read as the same family -- and every styling defect found in this system so far was found by comparing a fresh render against a reference card. This makes that comparison one action instead of hand-writing a composition each time.
