...
- **Non-active combatants rest at 85%, and the active one grows back to full size as it breathes.** The trough of the pulse (92%) sits above that resting size, so the active combatant is larger than the rest at every point in the cycle rather than only at the top of it. Both the portrait and its health ring carry the scale, in lockstep: in Round mode they are one figure, and moving only one of them would pull them apart.
- **All of it is on the button's CONTENTS, not the button, and that distinction is the whole point.** Scaling the button was tried first and eats the strip: a transform shrinks the *drawing* but not the layout box, so every button kept its full footprint and the unused remainder read as an enormous gap between portraits - the opposite of what a size cue should cost on a row where space is scarce. Scaling the contents costs the row nothing, and the spacing between combatants stays exactly `--gap`.
...
- **The centring translate is load-bearing in the keyframes.** `.combat-portrait-image` is centred by `translate(-50%, -50%)`, and a keyframe setting `transform` replaces that whole value - omit it and the portrait jumps to the button's bottom-right corner for the duration.
