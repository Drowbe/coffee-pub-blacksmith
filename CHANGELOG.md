...
- **Roll Table Import now has full schema-aware authoring and filtered source design** (`scripts/registry-json-import-rolltables.js`, `scripts/utility-rolltable-import-lists.js`, `scripts/parsers/parse-rolltable.js`, shared importer window/template/style, Roll Table profile prompts, importer architecture/API docs): all six profiles support clean JSON and guided human templates plus structured prompt controls for result count, replacement, displayed formula, equal/manual weighting, duplicates, result images, and exact-reference/text-fallback policy. World/compendium Actor and Item profiles share typed name/CR/type/size/rarity/magical filters; the name filter explicitly explains that it performs a case-insensitive contains search. Configured compendiums are independently selectable, and prompts/guides receive only the filtered exact-name catalog. The query layer is UI-neutral for reuse by the planned plain-text Utility tab/API. World document results now resolve exact document ids across supported collections, while invalid weights, ranges, and overlaps fail visibly.
