...
- **The pulse is on the portrait, not the button, and that distinction is the whole point.** Scaling the button was tried first and eats the strip: a transform shrinks the *drawing* but not the layout box, so every button kept its full footprint and the unused remainder read as an enormous gap between portraits - the opposite of what a size cue should cost on a row where space is scarce. Inside the button, the spacing between combatants is exactly `--gap` and nothing else.
