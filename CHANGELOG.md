...
### Added - disposition is the combat bar's portrait button, and portraits can be square

- **Disposition colours the portrait button** (`scripts/manager-combatbar.js`, `templates/partials/menubar-combat.hbs`, `styles/menubar-combatbar.css`): friendly, neutral, hostile, or secret, read from `combatant.token.disposition` against `CONST.TOKEN_DISPOSITIONS` by the new `CombatBarManager.getCombatantDisposition`.
- **Why the button and not a ring.** This was tried twice before landing here. A stripe across the bottom of the container read as chrome that had come loose - a rectangle bolted under a stack of circles. A concentric ring outside the health ring belonged to the figure but made two rings around one small portrait, and the eye had to work out which circle it was being asked to read. The container already had a fill that carried colour on exactly one occasion, the active combatant's turn; giving that fill to disposition puts colour on a surface that was otherwise wasted and leaves the portrait a single ring.
- **Whose turn it is moved to the border**, which was free, plus a soft outer glow. Border and fill are now two properties answering two questions instead of one property that had to mean both. Hover stopped using the border for the same reason - it made hover and the active combatant indistinguishable the moment a pointer crossed the wrong portrait - and lifts brightness instead.
- **No portrait state may set `background` any more.** Hover, hidden, and defeated each used to paint over the container, which would now silently blank the disposition of whichever combatant was in that state. Hidden keeps its dotted border and opacity; defeated drains colour with `grayscale` and keeps the skull overlay. `filter` does not compose across CSS rules, so `defeated:hover` restates the pair rather than giving every portrait an always-on no-op filter.
- **Portrait Shape setting** (`menubarCombatPortraitShape`, `scripts/settings.js`, `lang/en.json`): Round or Square, per user, under Run the Game -> Combat -> Combat Menubar. Round wraps the portrait in a health ring; Square lays a health bar across the portrait's lower edge. The disposition fill is unaffected by it. The in-flight initiative drag ghost follows the shape too; it lives on `<body>` so it is told directly rather than inheriting.
...
- **Eight new design tokens** (`styles/vars.css`, `documentation/design-system/design-tokens.md`): a solid and a `-bg` fill per disposition, following the variant palette's stated-not-derived pairing. The module's own muted palette rather than Foundry's saturated canvas colours, because these render inside Blacksmith chrome rather than over the map. Neutral's fill is the lightest of the four because its hue is the brightest; equal alphas made it shout over the other three. `node tools/check-design-tokens.mjs` passes.
...
- **Verify live**: start a combat with a friendly PC, a hostile NPC, and a neutral NPC. Each portrait button carries its own colour, with a single health ring inside it. The active combatant has a bright border and an inner glow, and hovering any portrait brightens it without borrowing that border. Change an NPC's disposition on its token sheet - the button recolours without reopening the bar. Hide a token as GM, and kill one - both keep their disposition colour while still reading as hidden and dead. Take a combatant to roughly half HP and confirm the ring is clearly half-lit against a dark groove rather than ambiguous. Switch Portrait Shape to Square: the ring is replaced by a bar across the lower portrait, the same combatant reads half-full there too, and dragging a portrait to reorder initiative shows a square ghost. Check the two ends in both shapes - full HP and zero, where zero shows a full pulsing red reading and an NPC with enemy health bars hidden shows a full pale one. Run the combat bar size setting from smallest to largest in both shapes. Confirm a player sees the same colours and the words on the hover card, including on an NPC's limited card.

### Changed - Round and Turn share one badge

- **`shape` on the statchip kind** (`scripts/api-menubar.js`, `templates/partials/menubar-secondary-item.hbs`, `styles/menubar-widgets.css`): `'pill'` (default, the label-then-value chip as before) or `'badge'`, which shows the value alone in a small square-cornered box and renders neither label nor icon. Documented in `documentation/api/api-menubar.md`. A shape rather than a new kind, because the markup, the tone and emphasis classes, and the patch path are all identical - only the label's presence and the box's radius differ.
- **Ordinary corners, not a capsule.** The badge was a stadium first, with a minimum width equal to its height so a single digit came out as a circle. It read as a token and it spent real width on the round ends - on the one row where width is the scarce thing - to say nothing a square-cornered box does not.
- **The badge sizes to a minimum height rather than a fixed one**, and its fill carries its own corner radius instead of relying on being cropped by `overflow: hidden`. A fixed height clipped an enlarged `lead` part top and bottom rather than growing to hold it. In the ordinary case the box still matches the feature pill's height, so the row stays even.
- **Two new `valueParts` forms** (`api-menubar.js`, the item partial, `menubar-widgets.css`). `{ divider: true }` draws a seam and carries no text - it survives the empty-text filter that drops blank parts, since being empty is the whole of what it is - so two readings can share one box without spending a character's width on a separator that reads as content. `{ text, lead: true }` marks the principal reading and sets it larger; two numbers in one box are equals until something says which the box is for, and equals read as a list. `lead` is a modifier on a value and is ignored on muted or divider parts.
- **The combat bar's Round and Turn are now one item, `round-turn`** (`manager-combatbar.js`): the round leads, then a seam, then the turn and its total. Between them the words "Round" and "Turn" cost more of the data row than the numbers they named, and two separate boxes then spent the saving again on a second set of edges and the gap between them. This is the one readout nobody needs told: its position never changes and the numbers only count up.
- **The tooltip carries the meaning and the values together** - "Round 4 - turn 2 of 6" - pushed with each update rather than fixed at registration, so the badge can never sit under a tooltip that does not mention its figures.
- **Fixed: `registerSecondaryBarItem` rejected a display item whose only content was `valueParts`.** The content guard tested `label`, `value`, and `image` and predates `valueParts`, so a chip stating its numbers in parts and naming itself only in its tooltip - exactly what this badge is - could not register at all.
...
- **Verify live**: open the combat bar in a fight. One squared badge holds the round number at a larger size, a seam, then "2 of 6", with no labels. Hover it - the tooltip reads "Round N - turn N of M" and tracks the numbers as turns advance. Advance past round 9 and confirm the badge widens rather than clipping. Run the combat bar size setting from smallest to largest and confirm the badge scales with the row and the enlarged round number is never clipped top or bottom.
