...
- **Veiled values: a card field can be readable by only some of its readers** (`scripts/manager-chat-cards.js`, `scripts/api-chat-cards.js`, `styles/cards-common-layout.css`). Anywhere a part accepts consumer text it now also accepts `{ value, readableBy, actorId, veil }`. `readableBy` is `'gm'`, `'owner'`, `'author'` or an array of user ids, allowlisted so an unrecognised name fails closed rather than falling through to everyone; `'owner'` resolves against the actor, and since a GM owns every actor it reads as "the GM and the owning player", which is exactly what a private roll wants from a single role. Resolution happens in `processText`, the one function every consumer-supplied string in the system already flows through, so all eighteen parts gained this without being touched. Two guarantees carry the whole thing and both are now enforced by `tools/check-card-contracts.mjs`: `mayRead()` refuses outright on the baked pass, and `post()` renders the stored snapshot with `baked: true` -- that snapshot is one string delivered to every client and shown until each one's own re-render lands, so a value revealed there is revealed to all. A veil holds the space the value occupied rather than collapsing, so a list of pending results does not reflow as they arrive. **This is presentation privacy and the API documentation says so in those words**: the value travels to every client in the message flags, because that is how the card re-renders, and a reader with a console can find it. It stops a number appearing on screen; it does not keep a secret.

- **Card buttons carry a weight, and the system places the primary one** (`scripts/manager-chat-cards.js`, `templates/parts/part-actions.hbs`, `styles/cards-common-layout.css`). A button in an `actions` part takes `variant: 'primary' | 'secondary' | 'critical'`, defaulting to `secondary`; the names match `.blacksmith-window-btn-*` in `styles/window-template.css` so one vocabulary covers cards and windows both. Colours differ deliberately -- a window's chrome is dark and a card's ground is parchment, and the window greens read as a foreign control dropped onto a card. `primary` fills with the colour `secondary` writes in, deriving from `--blacksmith-card-button-text`, so all nine card themes get a primary button that belongs to them without naming a new colour, and hover is a brightness step for the same reason. The variant goes through an allowlist before it reaches the class attribute, for the reason `safeColour()` exists: `class="card-button-{{variant}}"` unchecked lets a consumer name any class on the page. `disabled` had no styling at all before this -- the attribute was emitted and the button looked live. The primary button now renders rightmost whatever order the caller listed it in, because position carries meaning and leaving it to each module means a player hunts for the confirm action on every card. The `actions` part also takes `layout: 'inline' | 'stacked'`, defaulting to `inline`: stacked gives every button its own full-width row, for cards offering a list of alternatives rather than a confirm/cancel pair. The primary button is last in both layouts, so changing the layout does not move it -- bottom of a stack and right of a row are the same end of the list. `tools/check-card-contracts.mjs` gained a check that every name in both allowlists is actually styled.

- **Chat cards are now composed from parts rather than written as HTML** (`scripts/manager-chat-cards.js`, `scripts/api-chat-cards.js`, `scripts/cards-blacksmith.js`, `templates/parts/`, `styles/cards-parts.css`). `chatCards.post({ moduleId, parts, ... })` takes a composition and its data; Blacksmith renders every part from its own template and sends the message. Fifteen parts -- header, identity, image, meter, band, tiles, section, prose, pips, rows, badges, panel, notes, actions, richtext -- compose every card in the suite, including the ones sibling modules build today. Parts are named for their shape rather than for what goes in them: `tiles` is a grid of caption-over-value boxes, not "ability scores"; `rows` is a list of thumbnail-label-trailing rows, not "conditions". The first pass named three parts after the reference card each was found in and two were already wrong within a day -- the one called `status` was carrying monsters and player awards, and `actor` was carrying "Game Master". That naming test also merged two parts into one: `entities` and `status` were the same shape differing only in which optional fields were filled. The library is deliberately closed: a module composes built-ins and cannot register a part of its own, because a registration hook is exactly what lets per-module markup back in. The problem this solves is measurable rather than theoretical -- 84 posting sites across the suite, 21 card templates, five modules writing card HTML inline in JavaScript, and `templates/cards-common.hbs` and Squire's `templates/chat-cards.hbs` forked from each other with 231 lines diverged, the same invalid `visibility: none` on line 1 of both, and the same transfer described in two different sentences.
...
- **`node tools/check-harness-paths.mjs`** — asserts that the harness will actually work: every path it loads at runtime exists, every suite on disk is registered to run, every relative import in a suite resolves, and every `expect()` call passes its label first. That last one detects the argument inversion specifically -- a call whose first argument is not a string literal but whose last one is -- rather than flagging any non-literal first argument, because a label is legitimately built by an expression and a check that cries wolf gets ignored, which is worse than not having it. The harness fetches its library and suites over HTTP by absolute module path, so a rename produces a 404 at load time and nothing before it. That is the worst place to have a failure: the harness is what you reach for when you doubt something, and one that will not load teaches you nothing about what you were doubting. It also catches the quieter case of a suite sitting on disk that nothing lists, which would simply never run.

- **A Chat Cards suite in the test harness** (`testing/suites/suite-chat-cards.js`). Eight headless checks assert what can be read back off a posted message -- consumer HTML is escaped, inline marks convert, enricher syntax survives, the composition and a concrete theme are stored, a caller's own flags do not displace the card payload, an unknown theme falls back, and every registered part renders to markup rather than to nothing. Each posts a whisper and deletes it, so the log is left as found. The interactive tier posts **one card per button** across eleven groups. One at a time is the point: a card showing every part at once proves the parts render and hides everything about how they sit together, and every styling defect in this system was found by comparing one focused render against a reference card.

- **`node tools/check-card-contracts.mjs`** -- asserts that a consumer cannot inject presentation into a chat card. Two contracts, one principle: prose is escaped before anything else touches it, and caller-supplied colour is confined to data-visualisation parts. The second is the one most likely to drift, because every future part will have a moment where passing a colour looks harmless -- so the check holds the complete allowlist of parts permitted to take one, and fails if any other sets a colour from its template context. It loads the real functions out of `scripts/manager-chat-cards.js` rather than copying them, so it cannot drift from what it guards.

### Changed

- **The skill check card is composed from parts** (`scripts/cards-skill-check.js`, `templates/card-skill-check.hbs` and `styles/cards-skill-check.css` both deleted). `composeSkillCheckCard` is a pure function -- state in, parts out, no globals and no DOM -- so the same data always produces the same card and the harness asserts it headlessly rather than a person setting up a live roll to look at it. All three render sites go through one `skillCheckMessageData()`, so the create path and the update path cannot drift; the old template had two identical eighty-line actor blocks, one per group, for exactly that reason. The roll buttons became a registered action rather than listeners attached on every chat render, which matters because the card is rebuilt after every roll and a missed re-attach left a dead button. Two defects in the old template are repaired on the way: it compared `this.actorId` against `requesterId`, but `requesterId` is a USER id and `actorId` an ACTOR id, so `selfroll` revealed a total to nobody at all and `gmroll` collapsed to the GM-only branch. The roll mode now selects a `readableBy` instead, resolved in each reader's browser. Deleting the stylesheet was checked the same way `cards-xp.css` was: every `cpb-roll-*`, `cpb-versus-*` and `cpb-skill-check-*` rule had its only emitter in the deleted template, and the windows' `.cpb-actor-name` is styled by `styles/window-skillcheck.css:636`, unscoped and unaffected. Old chat history loses the styling of past skill check cards, per decision 8 of the card plan.

- **Parts gained clickable rows and band tooltips** (`templates/parts/part-rows.hbs`, `templates/parts/part-band.hbs`, `styles/cards-parts.css`). A row item with `action` and `clickable` makes the whole row the target rather than putting a small button beside it -- an actor waiting to roll is a thing you click. It takes the button tokens for its edge, weight and text so it reads as an affordance without restating a literal. A band accepts `tooltip`, and a row's tooltip now renders whether or not the row is clickable: the contest scores behind a winner, and the formula behind a total, were both tooltips on the card this replaced.

- **Announcement themes are replaced by a `-dark` variant of every theme** (`scripts/manager-chat-cards.js`, `scripts/api-chat-cards.js`, `styles/cards-common-themes.css`, `styles/cards-common-layout.css`). `announcement-green`, `announcement-blue` and `announcement-red` are gone, along with `postAnnouncement()`, `getAnnouncementThemes()`, `getAnnouncementThemeChoices()` and `getAnnouncementThemeChoicesWithClassNames()`. Each of the six themes now has a `-dark` partner -- `red`/`red-dark`, `blue`/`blue-dark` and so on -- that changes exactly one thing: the card header, filled with a dark ground and light text. Everything else comes from the ordinary theme, which is enforced by structure rather than discipline: a `-dark` class shares its partner's token block by sitting in the same selector list, so there is only one rule that is the variant. The announcement themes darkened the whole card and then left every text token at its light-theme value, which put dark brown body text on near-black; only the header had ever been given a light colour, which is the tell that only the header was ever used. Their three dark grounds are preserved exactly as the `-dark` grounds of blue, green and red. The card header also became a band in EVERY theme rather than only the dark ones: an ordinary theme tints it faintly with its own accent so it reads as a header rather than as the first line of the card, and a `-dark` variant fills it. Both come from one token, `--blacksmith-card-header-bg`, so there is a single rule for the header and the theme decides how loud it is -- which is what reduces each `-dark` variant to two token overrides. A header with nothing composed under it takes the fourth edge as well and rounds all four corners, so a header-only card is the band rather than a band above a strip of empty ground. Also collapsed a duplicate `padding` and `margin` on `.blacksmith-card`, each declared twice in the same rule and overridden two lines later.

- **Rich text is governed by the card, headings included** (`styles/cards-common-layout.css`, `scripts/blacksmith.js`). Only `h1`, `h2` and `h3` were styled; `h4` to `h6` fell through to Foundry's defaults, so a journal page could render a deeper heading larger than the one above it. All six are now one ramp, each step smaller than the last, set in the card header's own face so a heading continues the header's voice rather than starting a second one, and `h1` lost the `display: inline` that made a journal `h1` run into the paragraph after it. `h5` and `h6` recede, since a journal uses them as sub-text as often as as headings. Foundry core underlines headings and draws horizontal rules in its own weight and colour, both far heavier than anything else on a card; headings now clear that border and `h3` draws its own hairline from a new `--blacksmith-card-rule` token, which the `hr` uses too. List markers take the card's accent colour rather than the text colour, and lists indent from the marker so a two-digit number stays in the text column. Tables were being painted by dnd5e rather than by us: it fills `table thead` with a red gradient and stripes rows at a specificity our rules could not reach, so a header rendered maroon whatever the card theme and our row token did nothing -- the translucent fill we set was layering over that gradient instead of replacing it. Those rules now run through `.blacksmith-part-richtext`, which outranks it honestly rather than by trick, since a table can only reach a card through that part. Table text sits a step below body text and a header sits below its own data, because a header labels a column and the values are what the reader came for. Nothing in a table breaks mid-word any more: the card's `overflow-wrap: break-word` inherited into cells and turned "Roll" into "R / oll" in a squeezed column, so cells opt out -- prose has room to wrap and a narrow column does not. Headers truncate instead, and a new render pass measures every element the cards deliberately clip -- table headers, button labels, badges, both tile lines, row labels -- and attaches a `data-tooltip` carrying the full text to the ones actually clipped. Measured rather than assumed, so a header that fits is not saddled with a tooltip repeating what is already visible, and deferred a frame because `renderChatMessageHTML` can fire before the message is in the document, where every width reads as zero.

- **Tables in rich text are governed by the card** (`styles/cards-common-layout.css`). `.blacksmith-card table` had no `border-collapse`, so every cell drew its own edge beside its neighbour's and all internal rules rendered doubled -- which is why a table read heavier than everything around it. It now collapses, spans the full width so a journal table set in pixels cannot overflow a chat card, and takes its colour from four new tokens instead of the hardcoded parchment values `#BDB7A0`, `#E5E1D5`, `#F3F1E9` and `#CAC5B0`, which ignored every card theme and rendered a pale beige slab on the dark ones. Three dead declarations went with it: a `font-size` on the table element that every th, tr and td below it overrode, and `padding` on `tr`, which no browser applies. Four `0px` borders on `.blacksmith-card pre` were removed for the same reason -- they drew nothing. The richtext harness card previously exercised only paragraphs, a list and a blockquote, so none of this was visible; it now covers headings, both list kinds, a table, a rule and preformatted text.

- **Card prose has two inline marks, not three; the section header and blockquote were restyled** (`scripts/manager-chat-cards.js`, `styles/cards-common-layout.css`, `styles/cards-parts.css`). The inline code mark is gone: a chat card does not have inline code in practice, and the mark carried the lifted-out code-span machinery that protected the asterisks inside it -- the fiddliest part of the pipeline, and the part that once shipped literal control characters into the source file. A backtick is now an ordinary character. A code BLOCK, if one is ever wanted, belongs in the prose block types. Separately, a section header now takes the body text colour rather than a second one, since a divider marks where you are rather than competing with the prose beneath it; and a blockquote is a left rule and italic text rather than a filled, bordered panel. Neither `<blockquote>` nor `<code>` had ever been styled inside a card, so both were inheriting whatever else was on the page. The blockquote's source turned out not to be the system but Scribe, whose `.message-content blockquote` styles every blockquote in every chat message in the world; it ties on specificity with `.blacksmith-card blockquote` and won on load order, `scribe` sorting after `blacksmith`. The card rule is scoped through `.section-content` so it wins on specificity rather than on alphabetical luck. Scribe's global reach is recorded in `TODO-GLOBAL.md` against its migration.

- **A trailing row value is quiet by default and opts into prominence** (`styles/cards-parts.css`, `templates/parts/part-rows.hbs`). Two treatments exist in the source cards and they disagree: `.cpb-roll-total` is 1.2em roboto-slab because on a roll card the number is the content, while `.xp-gained` is 0.85em/900 sans because there the name leads and the award annotates it. The roll treatment had shipped as the only one, so migrated XP awards rendered larger and in a different face than they had. The quieter reading is now the default and `trailingSize: 'large'` is the opt-in, which also drops the roll card's roll-specific `#461313` from every trailing value in the suite -- colour comes from `tone`.

- **Everything that exists to check the module now lives in `testing/`** (moved from `utilities/` and `test-data/`). `testing/test-harness.js` is the entry point, `testing/harness-lib.js` the shared contract, `testing/suites/` the ten suites, `testing/data/` the importer fixtures, and `testing/preview-chat-cards.js` the visual preview. `utilities/` keeps only what it should have kept all along -- one-off actions a person performs, like repairing stats data or fetching monster data -- so the two are told apart by what they are for rather than by remembering which file went where. The harness `BASE` path and every cross-reference in the live docs were updated; released `CHANGELOG.md` sections were deliberately left pointing at the old paths, because they record where those files were at the time. `CLAUDE.md` now carries a table separating `tools/`, `testing/`, `utilities/`, `themes/`, `packs/`, and `node_modules/` by who runs them and where.
...
- **A `pips` part, for discrete state slots** (`templates/parts/part-pips.hbs`, `styles/cards-parts.css`). Groups of filled and empty slots around an optional centre marker, which is the click target when the card supplies an action. Built against Crier's death-save widget and reusing its exact values -- 13px rounded squares, dark green and dark red, both groups filling outward from the centre -- so that widget is the same object after migration as before. Named for the shape, not for death saves: hit dice, charges, ammunition, and legendary actions are the same thing with one group and no centre.

- **A named animation vocabulary shared by every part** (`styles/cards-parts.css`). `shake-x`, `shake-y`, `pulse`, `glow`, applied as `.cpb-anim-{name}`, suppressed under `prefers-reduced-motion`. A card names a **motion, never a meaning**: a critical hit is `tone: 'positive'` with `animation: 'shake-y'`, but the part does not know that pairing, so the same motion is available to a crafting success or a countdown. The existing crit and fumble rules bound the two together, which put a d20 rule inside the presentation layer. The shake keyframes moved here from `cards-skill-check.css`, which later steps delete -- keyframes are document-global, so a definition sitting in a doomed file would have taken the animation down with it silently.

- **`rows` carry an outcome** (`templates/parts/part-rows.hbs`). `tone` (`success`, `failure`, `pending`) sets colour and `emphasis` promotes it to a filled, bordered treatment. Five d20-shaped states in `cards-skill-check.css` -- pending, success, failure, critical, fumble -- turn out to be two ideas: what happened, and how much it matters. Nothing in the vocabulary assumes a die was rolled.

- **`image` takes stacked overlays** (`templates/parts/part-image.hbs`). Crier lays a blood layer over a character portrait, selected by remaining hit points; an overlay is an ordinary image drawn on top, not a special case.

- **`section` takes `align: 'center'`** (`templates/parts/part-section.hbs`), which drops the rule. That is the contested-roll separator ("Arcana - VS - Arcana"); a `versus` part would have been dead the first time anything else wanted a centred divider.

- **The `tiles` part states its column count** (`templates/parts/part-tiles.hbs`, `scripts/manager-chat-cards.js`). It defaults to the number of items, capped at six. An auto-fitting grid sized six ability scores at exactly the width where only five fit, so the sixth wrapped onto a row of its own -- found on the first live render.

- **`rows` carry an optional trailing value as well as an optional button** (`templates/parts/part-rows.hbs`, `styles/cards-parts.css`). The XP card needs a portrait, a name, a sub-line, and a value on the right; the reference cards needed a button in that position. Both fit the same part.

### Fixed

- **The card header band sat outside the card's border** (`styles/cards-common-layout.css`). The band bleeds to the card's edges with a negative margin cancelling the card's padding, but the padding was `0.5em` on the card and the cancelling margin `0.5em` on the header -- and `em` resolves against the element's own font-size. The header is `1.5em`, so the card inset 6.5px while the header bled 9.75px, overshooting by about 3px on every side and pushing the band past the border, which no amount of radius tuning could fix. The card's inset is now the absolute `--blacksmith-card-padding` token and the header's margin is `calc(-1 * ...)` of it, so the two cancel by construction. Separately the band used the card's full 5px radius while sitting 1px inside its border; `border-radius` applies to the border box, so the curve left on the padding box is smaller by the border width, and it now uses a `--blacksmith-card-inner-radius` token.

- **Text no longer pushes a card past its own edge** (`styles/cards-common-layout.css`, `styles/cards-parts.css`, `templates/parts/part-actions.hbs`). One rule, two halves: a structured single-line label ellipsises, because the box it sits in has a shape worth keeping; prose wraps. Button labels, badges and both tile lines now truncate rather than run off the card -- a stacked button reading "SCATTER -- A DELIBERATELY LONG LABE" was the visible case, cut off mid-word with no ellipsis. The button fix needed three parts, because `text-overflow` needs a box to work in and a bare text node in a flex container is an anonymous flex item that never gets one: the label is now its own span, that span carries `min-width: 0` so it may shrink at all, and the button's own floor became `min-width: min(100%, max-content)`. The plain `max-content` this replaced solved only half the problem -- it stops flex squeezing a button until its text wraps, but it also forces a button whose label is longer than the card past the card's edge. Tiles ellipsise for a second reason: a value wrapping to two lines makes one cell taller than its neighbours and breaks the grid. Cards also gained `overflow-wrap: break-word`, so a single unbroken token in prose -- a URL, a pasted id -- wraps instead of overflowing.

- **The XP card lost three of its own styles when it was migrated** (`scripts/xp-manager.js`, `styles/cards-parts.css`). Its awarded-XP green (`#18520b`), the muted italic marking a player who was not in the combat, and the level-up emphasis all vanished into generic row styling, because the card was rebuilt from whichever parts existed rather than from the stylesheet it replaced. Nobody would have filed this: the card looked plausible and was simply worse. Awarded XP now carries `tone: success` -- which already resolved to exactly `#18520b` and was sitting unused -- a non-participant carries `tone: pending`, a level-up sets `emphasis`, and player portraits crop square as `.player-portrait` did. Found by reading `cards-xp.css` end to end rather than by looking at the card.

- **A large band rendered at 1.17em instead of 1.3em** (`styles/cards-parts.css`). The size was applied to a child of the band, which is itself 0.9em, so the two compounded. The source `.cpb-card-section-subheader` is 1.3em of the card. Corrected to resolve against the card rather than the band.

- **Row thumbnails cropped art that should not be cropped** (`styles/cards-parts.css`). `object-fit: cover` takes the centre and discards the edges, which is right for a portrait and wrong for token and item art, where the subject is centred and the edges carry it. `contain` is now the default and `cover: true` is the portrait case, following Bibliosoph's `.encounter-monster-img` and the XP card's `.player-portrait`.
...
- **`styles/cards-xp.css`**, and its `@import` in `styles/default.css`. Every rule in it was scoped under `.cpb-xp-card`, a class no template or script emits since the XP card moved to parts, and the one unscoped rule (`.cpb-xp-player-awards`) has no emitter either -- so nothing in the file could match anything. A note in `TODO.md` had concluded the opposite and blocked the deletion, on the grounds that `player-portrait`, `player-list` and `cpb-actor-name` still appear in the XP and stats window templates. They do, but the rules naming them all require the `.cpb-xp-card` ancestor and so never applied to those windows; the windows take their styling from `window-stats.css`, `window-xp.css` and `window-skillcheck.css`, which is where it already was.

- **The Object Link Style setting and its stylesheet** (`objectLinkStyle` in `scripts/settings.js`, `updateObjectLinkStyles()` in `scripts/blacksmith.js`, and `styles/links-themes.css`, now deleted along with its `@import` in `styles/default.css`). The setting offered seven repaints of Foundry's own content links -- green, red, blue, light, dark, plain text, or none -- by writing ten `--a-content-link-*` properties onto the root element, which `links-themes.css` then applied to `a.content-link` and `a.inline-roll` throughout `.message-content` and `.journal-page-content`. Restyling a core affordance everywhere it appears is not this module's business: a content link is Foundry's, players learn it once, and a card that renders one should show the link the reader already recognises. It also reached well outside any Blacksmith surface, repainting links in journals and in every other module's chat messages. Removing it left the Quality of Life heading with nothing under it -- `coreLoadingProgress` only looks like it belongs there, since it registers during `init` and renders above every heading -- so that heading went too, along with all four localization keys.

- **The collapsible chat-card CSS** (`styles/cards-common-layout.css`, 63 lines). `.collapsible-content`, `.section-content.collapsed`, `.collapsible .summary`, and `.card-header.collapsible` had no caller in any module in the suite -- the one `collapsible-content` element that exists sits in a GM Notes sheet, outside any `.blacksmith-card`, so the card-scoped rules never reached it. Two selectors looked dead and were not: `.card-header:not(.collapsed)` matched every header, since none is ever collapsed, so those kept their effect with the dead qualifier stripped. Cards deliberately do not collapse -- if content is important enough to include it is important enough to see, and the one case that warrants hiding, how a number was calculated, is served by `data-tooltip`.
...
- **The combat bar centres the combatant whose turn it is, instead of parking them at the edge** (`scripts/manager-combatbar.js`). The first placement after a client loads does not animate -- on a fresh client the strip starts at the far left and the active combatant may be fifty portraits along, and gliding there is a long trip past fifty identical portraits that says nothing; it should simply already be in the right place. Every turn after that eases, because a turn passing is a change worth showing. `ensureCurrentCombatantVisible` scrolled by exactly the amount the active portrait overhung the strip, so it came to rest flush against whichever edge it arrived from -- and since turn order runs left to right, that was always the right edge. Everyone still to act sat off screen, which made "who is up next" a question you had to scroll to answer. It now eases the portrait to the middle of the strip, spending half a width to answer that for free, and clamps to the scrollable range so combatants near either end settle against that end rather than leaving dead space beside them. Measured from `getBoundingClientRect` rather than `offsetLeft`, since the strip is not a positioned ancestor and the portraits carry a 0.85 scale -- what is centred is what is drawn.

- **Every statistic on the combat bar's data row is its own button** (`scripts/manager-combatbar.js`, `styles/menubar-combatbar.css`). The hover wash covered the whole statistics group and the click discarded which chip it landed on, so seventeen readouts presented and behaved as one target. That was accurate about the destinations -- they all open the Party Statistics window -- and it is about to stop being accurate, since the chips are being wired to experiences of their own. The wash now lands on each chip -- `.secondary-bar-item` inside the stats group rather than the group container, with the group's `:has()` kept only to scope the rule, since the group element carries no id of its own. Dispatch goes through `CombatBarManager.openStatDestination(menuBar, itemId)`, one seam that receives the chip's registered id and currently sends every one of them to the same window. Nothing a user can reach changes destination; what changes is that the row stops teaching them it is a single control, which would have cost more to unteach later than it costs to state correctly now. The row's no-affordance rule is unaffected -- it forbids chrome that promises a click and delivers nothing, and these clicks are real.
...
- **The combat bar's scroll arrows move a page at a time, and actually feel like 220ms** (`scripts/manager-combatbar.js`, `styles/menubar-combatbar.css`). A click used to advance exactly one portrait, so crossing a twenty-combatant fight took twenty clicks. It now moves the visible width of the strip less one portrait -- nearly a full screenful, with one portrait of overlap left as an anchor so you can see where you came from. The step is measured from `clientWidth` and the real portrait width rather than being a fixed count, because the bar is as wide as the user's screen and portrait size is a setting; a fixed three-portrait step would still crawl on a wide bar. It is floored at one portrait, so a strip only wide enough for a single combatant still moves.
  ...
  The lag was separate and had nothing to do with the step size: `.combat-portraits` carried `scroll-behavior: smooth` while the manager's own easing writes `scrollLeft` once per frame. The browser answered each of those writes by starting its own animation toward it and re-targeting on the next frame, so the hand-rolled 220ms cubic arrived as a much longer drift. The CSS property is gone; the JS owns the animation.

### Fixed

- **Whose turn it is, visible** (`styles/menubar-combatbar.css`). The active combatant was marked by size alone, and the size barely moved: the pulse ran between 0.92 and 1 against a resting 0.85, so it stayed *above* its neighbours at every point in the cycle. That sounds like the safer reading and is in fact the weaker one -- a size that never crosses its neighbours' is a size you have to compare rather than notice -- and on a portrait this small, in a fight of sixty near-identical cultists, it was not findable at all. Two changes together. The pulse now straddles the resting size instead of clearing it, running 0.8 to 1 against a resting 0.85 -- the trough named once as `--portrait-pulse-trough` and derived from the resting scale, so it follows that scale rather than restating it and cannot differ between the two ends of the cycle -- which makes the active combatant swing visibly smaller and then visibly larger than everything around it twice a cycle. It also runs slower, at 3.5s rather than 2s, through a new `--combat-pulse-tempo` that the two dead pulses read as well: the three deliberately share a beat, because a dead combatant taking its turn runs both at once and on different periods they slide against each other and read as a fault. Declaring the duration once is what stops that happening the next time one of them is tuned. And every portrait that is not taking its turn drops to 60 per cent opacity, returning to full on hover so the row stays readable.

  Opacity because every other channel is spent or already rejected: the fill belongs to disposition, a coloured border and inset glow were the turn once and were rolled back for competing with it, corner ticks were rejected after that, and an outer glow would be sliced flat by the strip's `overflow-y: hidden`. Stepping the others back leaves the active combatant drawn exactly as it was and removes the competition instead, which is the move `.defeated` already makes. Scoped with `:has()` so a strip with no active combatant is not a row of dimmed portraits with nothing lit.

- **The combat bar holds its right end while any initiative is still owed** (`scripts/manager-combatbar.js`). A combatant with no initiative sorts to the end of the turn order, and its portrait carries the die a player clicks to roll -- so on a long bar the portraits somebody still has to act on are exactly the ones stacked off the right edge, and centring the active combatant hid the only thing anyone was being asked to do. While any `.combat-portrait-initiative-dice` is present the strip parks at its right end instead. Checked before the active combatant is even looked for, since initiative is normally owed before there is a current turn at all, and held under a sentinel anchor so that leaving the state is the same comparison as any other change of anchor. A reader who scrolls away keeps their position, exactly as during a turn -- rolls keep arriving and each one would otherwise haul them back.

- **The combat bar no longer opens with the active combatant stranded off screen** (`scripts/manager-combatbar.js`). On a fresh client the strip could come up showing neither end of the turn order, with no way to tell which overflow the current combatant was in. Two causes. The post-render sequence restored the remembered scroll position *before* `updateCombatPortraitScrollArrows` applied `combat-portraits-overflowing` -- and that class switches the strip from `flex: 0 1 auto` to `flex: 1` and reveals the two arrows, so it changes `clientWidth` and with it the maximum scroll. The restore was therefore clamped against a maximum about to grow, and because the scroll listener writes the clamped result straight back, the loss was permanent rather than momentary. The class is now applied first. Second, `ensureCurrentCombatantVisible` returned as soon as the turn had not changed, so a placement computed against a not-yet-settled layout was never revisited; it now also corrects a current combatant that is genuinely clipped out of view, silently and without animating. That correction stands down for the rest of the turn once the reader scrolls the strip themselves, by arrow or by horizontal wheel, so it cannot drag anyone back from looking ahead.

- **The combat bar's portrait strip no longer lurches, stutters, or re-scrolls while a turn is in progress** (`scripts/manager-combatbar.js`). Two defects with one root: the strip's scroll position lived in the DOM, and the DOM does not survive a fight.

  `renderMenubar` in `scripts/api-menubar.js` removes the whole menubar element and re-inserts it whenever the structure fingerprint changes, and for a bar with its own template that fingerprint is `JSON.stringify(secondaryBar.data)` (`_secondaryBarStateSignature`) -- which for this bar contains every combatant's health. A single point of damage therefore replaces the strip with a new element at `scrollLeft` 0, and during a fight that happens constantly: damage, effects, hidden state, disposition, timers. The visible result was the bar snapping back to the start and animating forward again after every hit.

  Worse, the easing helper captured the element it was given and wrote `scrollLeft` to it for the whole 220ms. A render landing mid-animation left it animating an orphan already removed from the document, while the strip on screen sat wherever it had last been put -- so a single glide was cut into pieces and restarted, which is what made turn advancement stutter and appear to re-scroll from where it had already been.

  The position is now the manager's: `_portraitScrollLeft` is written every animation frame and by the strip's scroll listener, and `restorePortraitScroll` puts it back at the top of the post-render `requestAnimationFrame`, which runs before the frame is painted. The easing helper is replaced by `easePortraitScrollTo`, which takes an absolute destination rather than a delta -- a delta stops meaning anything once the strip has been rebuilt under it -- and re-resolves the element on every frame instead of holding it, so a rebuild mid-glide is invisible. It declines to restart when a glide toward the same destination is already running, which is what the rebuild path would otherwise do on every render. `resetPortraitScroll` clears all of it on `deleteCombat`. The bar still rebuilds as often as it did; it no longer moves when it does.

  Re-centring is also gated on the turn actually changing: `ensureCurrentCombatantVisible` compares the active `data-combatant-id` against `_centredCombatantId` and returns otherwise, so damage no longer drags the view back from wherever the reader had scrolled it, and manual scrolling with the arrows survives a rebuild and holds until the next turn.
...
- **The combat bar's three context menus reorganised, with the gaps filled** (`scripts/manager-combatbar.js`, `templates/partials/menubar-combat.hbs`, `styles/menubar-combatbar.css`).
  ...
  **Ordered and divided**: start the fight, then what is on the canvas, then what those things are worth, then the record -- separated by the module's own `menu-divider`. Headings naming the groups were tried and removed: beside buttons that already carry their own labels they read as clutter, which is the same reason the bar's registered readouts have no banners. A divider with nothing on one side of it is dropped, so a world without the Quick Encounter tool does not get a rule against the bar edge.
...
- **The party bar is gone; its contents moved to where they belong** (`scripts/api-menubar.js`, `scripts/manager-combatbar.js`, `scripts/window-toast-send.js`, `scripts/manager-reputation.js`). It was a second toggle hiding four buttons and two readouts, half of which were shown elsewhere already.
  ...
  **Send Toast became a left-zone menubar tool.** Left-click composes, right-click lists the saved quick toasts -- the same shape the Notes tool uses. That folds in the separate **Quick Toast** button, which was a second icon for the same capability, and `MenuBar.showQuickToastMenu` goes with it: the menu is now built in `window-toast-send.js` where the template helpers already live.

  **Vote moved into the party leader's own menu**, under the crown. Calling a vote is the leader's power, so it lives with the leader rather than on a bar. The menu now opens for the elected leader as well as the GM -- GM-only would have taken the capability away from the one person it is for -- and shows them that one entry alone. Choosing who leads, whether by vote or by picking someone, stays the GM's: a leader calling a vote for leader from their own menu is not a say they should have. The crown renders as an active control for a leader instead of read-only, gated on the same predicate the menu is.

  **Party health and reputation were dropped rather than moved.** The combat bar already carries party health, and reputation is now read through the API by whoever wants to draw it -- so `ReputationManager` no longer registers any UI. It keeps the value, the scale, the chat cards, and the change broadcast.

### Fixed

- **A player could not change who can see their own note** (`scripts/manager-notes.js`). Removing themselves, or sharing with anybody, failed with "The ownership field may only be modified by a GM or Assistant GM user." `ownership` is a `gmOnly` field (`DocumentOwnershipField`, `common/data/fields.mjs`) and the server enforces it, so owning the note is not enough and no permission we can grant would help. Ownership writes are now delegated to an online GM over socketlib, the same way `PinManager.requestGM` handles writes players cannot make; the GM side re-checks that the target really is a note rather than trusting the message, since it runs with GM authority. Ownership is also split out of the rest of the update -- including it in a player's write made the *whole* update fail, taking the title and body with it -- and a player's note is created first and given its ownership immediately afterwards, because a create carrying the field is refused the same way.

- **Custom Vote did nothing** (`scripts/window-vote-config.js`). Submitting a filled-in custom vote silently returned: the handler read `dialog.form`, and DialogV2 exposes no `form` property, so an early guard tripped with no error and no vote. Foundry's own documented accessor is `button.form`, the native button-to-form association. The same mistake was in three timer dialogs in `scripts/api-menubar.js`, where every field read came back empty; `scripts/manager-vote.js` already had it right and carried a comment saying so.

- **Deploying the party asks how** (`scripts/utility-party.js`). `deployParty()` now offers grid, circle, line, scatter, and sequential at the moment of deployment, each with a one-line description of what it does, and remembers the choice as the next default. The **deployment-pattern button has been removed from the party bar** along with the setting-change listener that kept its label current: it cycled a world setting, which meant the answer to "how will this deploy" lived somewhere other than the deploy button and had to be looked up before every use. The pattern is a per-deployment decision, so it is asked per deployment. `deployParty({ pattern })` skips the picker and `deployParty({ prompt: false })` uses the saved default, so a scripted caller is never blocked on a dialog. The encounter toolbar's own cycle control is untouched.

  **It also only deploys members who are not already on the scene.** Deploying the party over the top of itself made duplicate tokens whose only remedy was clearing everyone and starting again. Presence is matched on `token.actorId`, the same test `clearPartyFromCanvas` uses to decide what to remove, so the pair round-trips rather than drifting into duplicates or leftovers. Deploying when everybody is already placed says so and does nothing.

  **The picker also asks whether to include player-owned NPCs** -- familiars, summons, companions -- and remembers the answer in `partyDeployIncludeOwnedNpcs`. The checkbox appears only when there are owned NPCs not already on the scene. Group actors are excluded from that roster deliberately: a dnd5e group actor is routinely owned by every player, so ownership alone would drag the party roster itself onto the canvas as a token.

- **Party actions on the API** (`scripts/blacksmith.js`). `deployParty`, `clearPartyFromCanvas`, and `isCurrentUserPartyLeader` are now public. All three already existed and were reachable only from Blacksmith's own menubar registrations, which meant a satellite wanting the same actions had to reimplement them -- a second, drifting copy of the deployment-pattern logic in one case, and a direct read of Blacksmith's `partyLeader` setting in the other. The two party actions keep their existing GM guard rather than gaining a second one at the boundary; `isCurrentUserPartyLeader` is wrapped so its internal `moduleId` parameter is not part of the public shape.

- **`blacksmith.partyReputationChanged`** (`scripts/manager-reputation.js`). Fires with `{ sceneId, reputation }` whenever party reputation changes. Emitted from Foundry's `updateSetting` rather than from `setPartyReputation`, deliberately: `Hooks.callAll` runs only on the client that called it, so emitting from the setter would reach the GM who made the change and nobody else -- the one client that already knew. Driving it off the setting update means it arrives on every client, and however the value was changed.
...
- **Notes** (`scripts/manager-notes.js`, `scripts/window-notes.js`, `scripts/window-note-editor.js`, `scripts/manager-note-placement.js`). A list with search and tag filters, an editor with collaborative rich text, canvas pinning, and one menubar tool -- left-click for the list, right-click for Quick Note, Open Notes, and your favourites. Adopted from Squire and rebuilt against the author's own requirements rather than ported; the system it replaces is inventoried in `documentation/plans/plan-notes-inventory.md` and the design in `plan-notes.md`.
  ...
  **A note's pin looks like a note out of the box** (`NOTE_PIN_DESIGN`, `scripts/manager-notes.js`). A 50px gold circle with a white border, the note icon, its title under the pin on hover, and its own event animations and sounds. Only the starting point -- the pin belongs to the note and the pin config window overwrites any of it -- and existing pins are left alone, since this is creation defaults rather than a migration.
  ...
  **Double-clicking a note's pin opens the note.** Registered against `PinManager` directly, filtered by `moduleId` and pin type through the registration options rather than by a test in the handler, and permission-checked before opening -- the same shape the journal pins use. A pin whose marker does nothing is a marker for something you cannot reach.
...
- **`supersedes` on `registerMenubarTool`** (`scripts/api-menubar.js`). A tool declares the ids it replaces; any listed id already registered is dropped, and any that registers later is refused. Both halves exist because module load order is not something either module controls, so a rule that only works one way round is not a rule. It solves the window in which a tool has changed hands and a user has updated one module but not the other, which would otherwise show two identical icons. Nothing declares it yet -- Blacksmith and Squire release together here, so that window never opens -- but the next extraction meets the same problem, and the surface is documented in `documentation/api/api-menubar.md`.
...
- **The Status Effects window is now Blacksmith's** (`scripts/window-status-effects.js`, with template and stylesheet). Conditions and Active Effects for one actor: toggle a condition, remove an effect, read the enriched description of the selected row. Adopted from Squire, 424 lines, the smallest of the moves and the only one that removed machinery rather than adding it.
  ...
  **It moved because of an inversion this repo created and then had to look at.** Blacksmith's Health window wanted a per-row conditions button, could not name a Squire window, and so advertised a capability id -- `blacksmith-status-effects` -- that a satellite had to claim, with the button rendering only if one did. A hub surface degrading because an optional module is absent is the coupling this whole effort has been removing; the id was a workaround for the window being in the wrong place. Blacksmith now registers that id natively, the conditional branch and the `isWindowRegistered` check are gone from the Health window, and the button always renders.

  Both description options are public and documented: `descriptionEffectId` names an ActiveEffect already on the actor, `descriptionStatusId` names a configured status that may not be applied yet. The second was nearly lost in the handover -- no Squire caller passes it, so anyone enumerating the contract from call sites would never see it. That is why the option set is spelled out in `documentation/api/api-window.md` rather than left to be inferred.

  Effect enumeration and condition labels go through `EffectsAPI`, so this window and the rest of the suite agree on what an effect is called. `getDisplayEffects` is deliberately **not** used: it enriches every effect's description, while this window enriches only the one selected row, so it would do N enrichments per render to display N names. The API's richest method not fitting its most obvious consumer is a finding about the API, and it is recorded rather than worked around.

- **`getHealthPercentForHP({value, max})`** (`scripts/utility-health.js`), matching the existing `getHealthSeverityForHP`. Asked for by Squire, whose adoption of `getHealthPercent` found four hand-rolled copies of `(value / max) * 100` that disagreed at the edges -- one guarded `max > 0`, one clamped, two did neither and rendered `NaN%`. One site could not use the actor-shaped entry point because a Handlebars helper receives the HP object and not the Actor that owns it, which left the arithmetic duplicated in the one place most likely to drift.

  It is now the primitive the others read from, so the same clamp is no longer written three times inside Blacksmith either. That collapse also fixed a case: `getHealthSeverityForHP` coerced a missing `value` to `0` and returned `dead`, painting a corpse indicator on an actor that was merely missing a field. Both `ForHP` helpers now return `null` when the pair is unusable.
...
- **Inventory mutation primitives -- `blacksmith.inventory`** (`scripts/api-inventory.js`, registered at `scripts/blacksmith.js:1000`): four methods that move items and coins safely. `grantItem`, `grantItems`, and `grantCurrency` put content onto an Actor from a compendium, loot table, or crafting result; `transferItem` and `transferCurrency` move it between two Actors. They own no workflow -- no sockets, no permission checks, no windows, no chat -- so approval, recipient selection, and domain rules stay with the consuming module. Surface in `documentation/api/api-inventory.md`, mechanism in `documentation/architecture/architecture-inventory.md`, both published.

  This consolidates eleven duplicate mutation sites across three modules, but deduplication is the weaker half of the reason. Every existing copy shares four defects that dnd5e 5.2.5 already defines correct behaviour for. Transferred items arrived still equipped and still attuned, because a raw `createEmbeddedDocuments(item.toObject())` skips the reset set dnd5e applies on its own drop path (`dnd5e.mjs:55332`) -- so a handed-over magic item consumed no attunement slot. Containers corrupted silently, because dnd5e stores containment on the child as `system.container` (`dnd5e.mjs:19028`), so moving a bag orphaned its contents against an id that no longer existed. Source actors were resolved with `game.actors.get`, which cannot resolve an unlinked token actor -- the form every corpse on the canvas takes. And quantity was re-checked in only one of four copies, so a stale client-side value could create the full requested amount on the target and delete the source stack.

  **Arrival flags are written by the primitive, in the same operation as the item, and this is not a convenience.** dnd5e recomputes encumbrance on every item create, update, and delete (`dnd5e.mjs:36073`) as a check-then-create against one fixed effect id with no lock (`:36226-36238`), and Foundry does not await that recompute from the promise `createEmbeddedDocuments` returns. Two sequential writes to one Actor -- even individually awaited -- both try to create `dnd5eencumbered0`, and the server rejects the second. Correct awaiting does not avoid it and neither does this layer's own mutex, because the recompute completes outside the critical section; reducing the operation to one write per Actor is the only fix available to anyone. Squire hit this in a live player-to-player transfer. It is also why `grantItems` exists rather than a documented instruction to serialise grants.

  Merging is now the default rather than a new row per arrival. A duplicate row is not a neutral outcome: item ids are referenced by favourites lists, tracked light sources, and container membership, so a second row quietly detaches from all of them. Eligibility is gated on item state rather than item type -- dnd5e stacks consumables only, which is why two identical daggers do not stack on a sheet -- by deep-comparing `name`, `type`, the whole of `system` minus `quantity` and the reset set, and the whole of `flags` minus caller-declared transient keys, plus a separate enchantment check since those are ActiveEffects and sit outside `system`. Comparison runs on `_source` data, never the prepared model, because derived values such as `uses.value` differ between identical items for reasons unrelated to identity. `_stats.compendiumSource` is a negative signal only: a missing source means unknown, and only two present-and-disagreeing sources block a merge -- measured in one live world, zero of 564 pack items carried a source while 278 of 281 of the same items in the world did, so any stricter rule would have made stacking a coin flip.

  Three further behaviours are deliberate and destructive to "simplify", all recorded in the architecture doc. Rollback is quantity-aware: deleting the target row after a *merge* destroys quantity the recipient already owned, and fails while looking like successful cleanup. The public methods hold the locks and call unlocked internal cores, because a public primitive calling another public primitive self-deadlocks on every transfer -- a hang with no error. And a lock waiter that times out chains its own release to the real holder rather than resolving immediately, since resolving early would let the next waiter acquire while the holder still holds.

  Scope is bounded on purpose: physical item types only (a raw create of a class or feat bypasses dnd5e's singleton and advancement flow), non-empty containers refused with a `contentCount` so a consumer can say how many items to unpack, same-actor transfers refused, and currency denominations never auto-converted -- exchanging coins to satisfy a payment is a table's house rule, not a mechanic. An unverifiable container is treated as packed, because a refused bag is a visible annoyance and an orphaned one is silent corruption.

  **Verified:** both invariant checks pass; `node --check` clean. Covered by a new harness suite (`utilities/tests/suite-inventory.js`, registered in `utilities/test-harness.js`) -- 18 headless checks including the quantity-aware rollback, lock serialisation, deadlock-free opposite-direction transfers, and the one-write-per-actor encumbrance assertion, plus 3 interactive checks for what a harness cannot reach. The suite creates and deletes its own throwaway Actors and never touches existing ones. **Passed 179/179 on 2026-08-09.** `transfer-items-write-count` reported source `{update:1, delete:1}` and target `{create:1}` for five items, which is the only direct evidence that batching works. Cross-client concurrency stays interactive because the mutex is per-client by design.

- **XP no longer loses a kill when the corpse is cleared** (`scripts/stats-adversaries.js`, read by `scripts/xp-manager.js`): resolution inputs are now recorded as combat proceeds instead of being derived from the final state of the combat tracker.

  `detectMonsterResolution` reads hit points to decide `DEFEATED` / `ESCAPED` / `IGNORED`. That is only safe while every token still exists: `Combatant#actor` falls back to the base prototype actor once its token is gone (`client/documents/combatant.mjs:84-87`), and a prototype is at full health because combat damage lives in the token's delta and dies with it. So a monster killed and then looted-and-cleared mid-fight re-derived as undamaged and earned nothing -- the row was still in the window, which is what made it easy to miss, just with the wrong resolution and a zero multiplier. Looting a corpse during a fight is ordinary practice, not an edge case: it declutters the canvas.

  The record stores hit points, maximum hit points and defeated state -- **never a resolution, multiplier or XP figure**. `detectMonsterResolution` still decides, so a GM correcting a resolution in the window, or a table changing its multiplier settings later, is not arguing with a frozen answer. It is keyed on **combatant id** rather than actor id, because an unlinked token carries the base actor's id and two tokens of one prototype are otherwise indistinguishable -- exactly the case this has to get right. It is persisted as a flag on the Combat document, because sessions stop mid-combat, and it is read back off the document the `deleteCombat` hook passes, which is what makes the award work once `game.combat` is null.

  Captured at three points, each covering what the others cannot: `preDeleteToken` (the last moment a token's hit points exist anywhere), `preDeleteCombatant` (a GM toggling a corpse out of the tracker deletes the combatant outright, losing the row entirely rather than just its resolution), and `updateCombat` (damage, a GM marking something defeated, a combatant added mid-fight).

  The adversary's **name** is preserved the same way and for the same reason. `Combatant#name` is a stored field that derives as `token?.name || actor?.name` when empty (`client/documents/combatant.mjs:159`), so for an unlinked token the fallback is the PROTOTYPE name -- an adversary placed from a "Cult Leader" prototype and renamed reverted to the prototype in the XP window as soon as its token was deleted. The name now comes from the live token while one exists, so a mid-combat rename is picked up immediately, and from the record only once there is no token left to ask.

  The name is stamped onto the Combatant as well, not just read from the record, because the **combat tracker** reverts too -- a moment after the corpse is cleared rather than immediately, since the derivation runs on the next data preparation. Writing the derived name into the field it derives into stops the derivation (`||=` leaves a populated value alone) and fixes both surfaces. A name already stored is never overwritten, since that is a GM's explicit rename.

  Two rules keep the record from destroying itself, both found by testing the reload case. A combatant whose token is gone can only produce a worse snapshot -- prototype hit points, prototype name -- so both capture paths skip it when an entry already exists; without that the periodic sweep overwrote good evidence on the round after a corpse was cleared. And every value is read before the first await, because `preDeleteToken` is not awaited by Foundry: the first version awaited a write and then read the name for the stamp, by which point it resolved to the prototype and wrote "Cult Leader" over the name it existed to preserve.

  Also corrected: `detectMonsterResolution` now consults `Combatant#isDefeated` directly and first. `defeated` is a stored BooleanField on the Combatant (`common/documents/combatant.mjs:56`) and survives token deletion and reload on its own, so reaching it only through the record made a correct answer depend on capture timing for no reason. The record earns its place on what is genuinely unstored: the display name, the hit points that separate `ESCAPED` from `IGNORED`, a CR fallback, and combatants deleted outright.

  One guard worth naming: the record writes to a flag on the Combat document and the periodic refresh is registered on `updateCombat`, so an unconditional write would be a loop with a server round trip per iteration. The record compares before writing, and the sweep separately ignores its own flag in the hook's `changed` argument. Both, because either alone leaves the code one edit away from a runaway write, and neither relies on Foundry short-circuiting a no-op update.

  Also fixed alongside: `getCombatMonsters` no longer silently drops a combatant whose actor cannot be resolved at all, and `getMonsterCR` falls back to the recorded CR in that case rather than returning 0, which read as "worth nothing" instead of "we lost its CR". CR still prefers the live actor when one exists, so a GM correcting a wrong CR applies -- deliberately the opposite precedence to resolution, where the live document is the thing that lies.

  The menubar XP path with no active combat now says in the console that it is listing canvas tokens rather than a recorded encounter. It was silently presenting the scene as the encounter.

- **Create Combat leaves the dead out** (`scripts/api-menubar.js`): the tool falls back to every placeable on the canvas when nothing is selected, so a new encounter started with the previous fight's bodies in the tracker. It now filters through `EncounterManager.canStillFight` and reports how many it skipped; if everything found is already out of the fight it says so rather than creating an empty combat. A GM who wants a corpse in the tracker still adds it deliberately.

- **Encounter CR badges stop counting corpses as threats** (`scripts/ui-journal-encounter.js`): four callers of `getPartyCR` and `getMonsterCR` omitted `onlyStanding`, which defaults to false, so difficulty ratings included the dead. The combat bar already passed it -- that asymmetry was the tell. The default is deliberately left alone: those functions are on the public API, and flipping it would silently change results for consuming modules. `canStillFight` carries the rules asymmetry, so a player character at zero still counts toward party CR (they are dying, not dead) while a monster at zero does not.

  **Verified:** `node --check` clean on every touched file; both invariant checks pass. New harness suite `utilities/tests/suite-xp-record.js` (registered in `utilities/test-harness.js`) with three headless checks -- the reported bug as an assertion (kill, delete the token, assert the resolution is still `DEFEATED` and base XP is non-zero), that the record stores no verdict fields, and that the flag really reaches the document source so a reload restores it -- plus interactive checks for Create Combat and the CR badges. **Passed 35/35 on 2026-08-09.**

- **Encumbrance Guard: a central mitigation for a dnd5e race** (`scripts/manager-encumbrance-guard.js`, wired in from `scripts/manager-libwrapper.js`): serialises and coalesces `Actor5e#updateEncumbrance` per Actor, so two writes to one Actor can no longer both try to create the same encumbrance effect. New `enableEncumbranceGuard` world setting, on by default, under Developer Tools.

  `updateEncumbrance` reads `this.effects.get(ActiveEffect5e.ID.ENCUMBERED)` and, when absent, creates an effect with that same fixed `_id` and `keepId: true` (`dnd5e.mjs:36226`, `:36235-36238`). Check-then-create, no lock. It runs on every item create, update and delete on an Actor and on the Actor's own update, and Foundry does not await it from the write that triggered it -- so the server rejects the second create with `The _id [dnd5eencumbered0] already exists`. The write itself succeeds, so it presents as console noise, which is why it can sit in a log for months.

  **Why centrally rather than per module.** A per-module fix removes only that module's contribution. Two different modules writing to one Actor in the same moment collide with neither at fault, so "no module trips it alone" is strictly weaker than "it cannot happen", and it decays the moment anyone adds a follow-up write. This does not replace the batching in `grantItems` and `transferItems` -- fewer writes remains better on its own merits -- it stops correctness depending on that discipline.

  **Coalescing, not only serialising.** Serialising alone would still run one recompute per write. Only the last one matters, because `updateEncumbrance` reads current state each time, so at most one runs and one waits and further calls collapse into the waiter. That is safe to do because the method accepts an `options` argument and never reads it; the latest options are carried through regardless. One visible consequence: fewer stacked encumbrance popups during a bulk change.

  Built to remove itself rather than become a patch nobody remembers: a `FIXED_IN_DND5E` version gate, feature detection on both `Actor#updateEncumbrance` and `ActiveEffect.ID.ENCUMBERED` with a logged refusal rather than a blind patch, registration through libWrapper so a conflict with another module is visible instead of last-writer-wins, a one-time activation log, and the setting. Keyed on Actor UUID rather than id, since a synthetic token actor carries the base actor's id and would otherwise queue every corpse from one prototype together.

  The catch is deliberately narrow -- only a message containing both `already exists` and `dnd5eencumbered`, and only as a backstop behind serialisation. Widening it would hide real failures in a path nobody watches, so a harness check asserts an unrelated failure still propagates.

  **Verified:** `node --check` clean; both invariant checks pass, including the settings-headings check after adding the setting. Three new harness checks (28 headless total) covering installation-or-a-stated-reason, six separate writes to one near-threshold actor producing zero rejections, and the narrow catch not swallowing an unrelated error. **Passed on 2026-08-09**, with the activation line present and no libWrapper conflict beside it. The guard was also switched OFF and reloaded, and `guard-collapses-recomputes` failed as it must -- the check that proves the guard does anything rather than assuming it, since nothing in a passing run with the guard on can substitute. Looting onto a near-encumbered player through Curator's window behaved the same as the harness path. Note that the older `one-write-per-actor` check now tests the guard rather than our write discipline, since with the guard installed no write pattern produces a collision; `transfer-items-write-count` is what proves the batching.

- **`blacksmith.inventory.transferItems` -- batched Take All** (`scripts/api-inventory.js`): moves several items between two Actors in at most two writes per Actor rather than two per item. Requested independently by Squire and Curator, and the argument for it is not ergonomics: dnd5e recomputes encumbrance on every write to an Actor, as a check-then-create against one fixed effect id with no lock, and Foundry does not await that recompute from the write that triggered it. A loop of single transfers therefore makes item count into the number of racing recomputes. Every create in a batch goes into one `createEmbeddedDocuments`, every merge into one `updateEmbeddedDocuments`, and the source is reduced with one batched update plus one batched delete.

  Validation is per item rather than all-or-nothing, so one packed container on a corpse does not stop the other rows being taken; each entry reports its own result, index-aligned with the request, with `itemId` on failures so a refusal can be attributed to a row. The same `itemId` twice in one call is refused with the new `DUPLICATE_ITEM` code -- two entries for one item make the per-entry quantity checks meaningless, since each validates against the full stack and together they could over-draw it, and summing them would guess at intent the caller can state exactly.

  Extracting the shared batch core also fixed a bug in `grantItems`: the merge-candidate search only looked at documents that already existed, so payloads queued earlier in the same call were invisible to each other and duplicate entries produced separate rows. A Take All over a corpse holding two identical arrow stacks would have split them. Entries now coalesce into a queued sibling, reported as `coalesced: true` with `merged: false` -- nothing that already existed was grown, but neither did they create a row of their own. Merges into pre-existing rows are batched into one update as well.

  Batch rollback follows the same rule as the single-item path and for the same reason: created rows are deleted, merged rows are decremented by exactly what was added, never deleted. It tracks what it did as it goes rather than inferring it afterwards, because once a merge has landed nothing in the document says how much of the stack arrived in that operation.

  **Verified:** `node --check` clean; both invariant checks pass. Four new harness checks in `utilities/tests/suite-inventory.js`, including a direct write-count assertion -- five items must cost at most two writes per Actor -- since the property the batch form exists for is invisible in the return value and its absence shows up only as intermittent console noise. Passed on 2026-08-09.

- **Token interaction claim registry -- `blacksmith.tokens`** (`scripts/api-token-interactions.js`, registered at `scripts/blacksmith.js:998`): a consuming module can now claim a canvas gesture on a token it does not own, so a player double-clicking a lootable corpse gets that module's window instead of the Actor sheet. This cannot be done with a hook, and the reason is worth stating because it is the whole justification for an API: Foundry evaluates the gesture's permission predicate *before* dispatching the handler (`MouseInteractionManager#can` at `client/canvas/interaction/mouse-handler.mjs:494`, with `clickLeft2` bound to `Token#_canView` at `client/canvas/placeables/placeable-object.mjs:792`, requiring LIMITED on the Actor). A player has no permission on a corpse, so nothing downstream ever runs and there is nothing to subscribe to. `Token#_onClickLeft2` renders the sheet directly and fires no hook.

  Implemented without adding any libWrapper wrapper and without touching a prototype. `PlaceableObject#activateListeners` stores the interaction manager's `permissions` and `callbacks` as plain own properties read per gesture (`mouse-handler.mjs:64-65`, `:294`, `:311`), and it has exactly one caller -- inside `draw()`. Blacksmith already wraps `Token.prototype.draw` and fires `postCoffeePubTokenDraw` once that manager exists, so the registry replaces entries on an individual token's own manager from there. With nothing registered, no hook is registered and no token is touched.

  `matches` resolves when the gesture fires rather than when the token draws. A draw-time decision is cheaper but wrong for the case the feature exists for: a creature that dies mid-session becomes lootable without redrawing, so its corpse would stay unclaimable until something redrew it. The claimable gestures are double-clicks, so per-gesture resolution costs nothing that matters. `matches` cannot be `async` -- the predicate is synchronous by Foundry's contract and a promise is truthy, so returning one would grant the gesture unconditionally.

  Three behaviours are deliberate and destructive to "simplify", and all three are recorded in `documentation/architecture/architecture-token-interactions.md` because none is recoverable from reading working code. A throwing claim handler suppresses the gesture and does **not** fall through to Foundry's handler, because `bypassPermission` may already have granted it and falling through would open the Actor sheet to a user who could not otherwise open it. An internal marker distinguishes our replacement from Foundry's real entry, because `draw()` rebuilds the manager and re-running the patch must capture the genuine original or the pass-through path calls itself. And the no-claim branch of the dispatch path re-evaluates the displaced predicate before falling through: Foundry checks permission and dispatches in two separate calls, so a claim can match during the check, grant the gesture through the bypass, and stop matching before dispatch -- which would otherwise run Foundry's handler on a gesture only the bypass allowed. That last one reads as a redundant permission check sitting beside Foundry's own, which is precisely why it would be tidied away.
...
- **The shared primary-button class no longer carries a fixed width** (`styles/window-template.css`): `.blacksmith-window-btn-primary` had `width: 300px`. In a flex footer that stretches the primary action across the whole row and squeezes the secondary one onto two lines, and it applied to every consumer of the shared window classes. Reported by Curator, who was resetting it in their own scope.

  The reset outnumbered the rule: `styles/dialog.css`, `styles/window-pin-config.css` and `styles/window-pin-layers.css` were all already overriding it back to `auto`, and `dialog.css` named the offending declaration in a comment. Neither of the other two variants (`-secondary`, `-critical`) carried a width at all, so the asymmetry was the tell. The width is gone from the shared class, the two overrides that existed only to undo it are deleted, and `dialog.css` keeps its `width: auto` because it sits with `flex: 1 1 auto` and `min-width: max-content` doing real work for a shared button row. A window that genuinely wants a wide primary button now sets it in its own scope.

  **Consumers overriding this can drop the reset.** Verify a window with two footer buttons still lays out correctly -- that is the case the fixed width was breaking and the one worth looking at.
...
- **Nameplate style "none" still renamed every token placed** (`scripts/manager-canvas.js`): with the feature switched off, `_onCreateToken` fell through to the write anyway and stamped the ACTOR's name onto the token. Usually invisible, because a token is normally created carrying its actor's name -- but it cost one document write per token placed, and it silently discarded a name a token was deliberately created under. It now returns without writing.
  ...
  **Verified:** `node --check` clean on all three files; both invariant checks pass. Live: `xp-record` went 29/33 (four name failures) to 34/35 to 35/35 on 2026-08-09.

- **Lifetime standings vanished from the combat bar on any actor change** (`scripts/manager-combatbar.js`): every chip fed by the party aggregate -- biggest hit, most fumbles, top MVP, the party totals -- disappeared when an actor was updated and did not come back until something unrelated re-rendered the row. In a fight that is every hit point change, so the standings were absent for most of the time anyone was looking at them.

  `updateActor` invalidates the party aggregate (`scripts/stats-party.js:38`, alongside `createActor` and `deleteActor`) and the same hook rebuilds the bar. That rebuild decides visibility through `getAggregateSync`, which returns null on a cold cache by contract, so every lifetime chip evaluated as hidden. `refreshReadoutItems` then rebuilt the aggregate and wrote the values, but writing a value cannot reveal a hidden item -- appearing is structural and only a render applies it, which the code said itself two hundred lines above.

  **The comment there was the reason this stayed invisible**, and it has been corrected rather than deleted: it claimed a re-render was already guaranteed because the standings change on `blacksmith.combatSummaryReady`. That was true of what changes the numbers and false of what makes the read return null. The invalidation set was wider than the re-render set, and everything in the gap failed silently. `refreshReadoutItems` now takes the bar and asks for a render when it had to rebuild the aggregate itself, behind a re-entrancy flag -- the second pass reads a warm cache and asks for nothing further, and the flag covers the case where another actor update invalidates in between, which is exactly what would otherwise make it chase itself.

  **Verified:** found by the harness, not by inspection -- `readouts/empty-state` reported ten chips as having data and not being rendered. It only reproduces in a full "Run All Headless", because the inventory and XP suites create and delete actors while the readouts checks run; the tab passes on its own with a warm cache, which is why per-tab runs had been green. **Full run passes 519/519 on 2026-08-09**, having been 500/513 before the fix. `node --check` clean; both invariant checks pass.

- **The readouts suite reported the combat bar as broken when it was correct** (`utilities/tests/suite-readouts.js`): it read the party aggregate through `getAggregateSync`, which returns null on a cold cache and starts a rebuild -- that is its contract, for renderers that cannot await (`api-stats.js:90`). Ten rendered chips therefore read as having nothing to report, and the rebuild landing mid-check re-rendered the row, which marked every newly-appearing lifetime chip as entering and rebuilt the entrance probe's node as an already-known item, so the probe lost its class and its neighbours gained one. All nine of the remaining unexplained failures in the 463/476 run were this. The suite now awaits `getAggregate`, and the entrance check settles the row before introducing its probe rather than measuring a re-render it caused itself. Readouts passes 34/34. No behaviour changed; the code under test was right throughout.
...
- **The shared context menu could not be dismissed in some windows, and Escape closed the wrong thing** (`scripts/ui-context-menu.js`, `styles/menu-context-global.css`): four fixes to a widely-used component, three of them reported from Cartographer's map-placement menu.
  ...
  **Images in the icon slot get a thumbnail treatment** (`styles/menu-context-global.css`): a hairline border, a small radius, and a faint backing fill, applied to `img` and `svg` only and never to the slot -- a bordered rounded box around a Font Awesome glyph looks like a mistake. The backing fill is not decoration: item and macro artwork is frequently a transparent PNG, and against the menu's own dark translucent panel it loses its edges entirely. Both treatments follow row state, warming on hover and dimming with a disabled row. Uses `--blacksmith-radius-sm` and `--blacksmith-surface-dark-3` rather than literals.

  The slot and `.context-menu-item-portrait` are both 24px deliberately. A generic `> img` selector outranks the portrait class on specificity -- two classes plus an element beats two classes -- so a differently sized slot would have silently overridden the portrait variant rather than coexisting with it, shrinking the combat bar's actor portraits from 24px to the slot size. Equal sizes make the specificity irrelevant, and the image rules use `max-width`/`max-height` rather than `width`/`height` so a variant class can still set its own size.
