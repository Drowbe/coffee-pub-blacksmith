...
- **Multi-contributor GM Notes sections** (`scripts/manager-gmnotes.js`, `scripts/api-gmnotes.js`, `scripts/ui-gmnotes-field.js`, `styles/notes-gm.css`, `documentation/api/api-gmnotes.md`, `documentation/architecture/architecture-gmnotes.md`): GM Notes now keeps the GM-authored General note separate from namespaced module annotations. Modules can store owned sections with `setSection()` or register live, read-only content with `registerProvider()` when the authoritative text already lives in module data. The shared `createField()` component renders General first, then persisted and contributed sections in one GM NOTES area; module sections are attributed and independently collapsible/hideable per user. General writes and clears preserve module sections, provider content is never persisted, provider failures are isolated, and the re-import contract now distinguishes General preservation from section merging. Verify live with Bibliosoph: mount one field on an Injury page, confirm “Running This Injury” appears beneath General without a duplicate standalone block, edit General without losing it, collapse/hide the contributed section, and confirm another provider can contribute without overwriting either.
