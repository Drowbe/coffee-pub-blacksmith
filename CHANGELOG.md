...
### Added - the combat bar shows disposition

- **A colour stripe under every portrait** (`scripts/manager-combatbar.js`, `templates/partials/menubar-combat.hbs`, `styles/menubar-combatbar.css`): friendly, neutral, hostile, or secret, read from `combatant.token.disposition` against `CONST.TOKEN_DISPOSITIONS` by the new `CombatBarManager.getCombatantDisposition`.
- **Drawn as an `::after`, not a bottom border.** The container's `border` is already spoken for by the hover state and the hidden-token state; a disposition colour competing with those would win or lose by rule order. The stripe dims on a defeated combatant rather than disappearing - the dead have no side, and a missing bar reads as a rendering fault rather than as a fact.
...
- **Four new design tokens** (`styles/vars.css`, `documentation/design-system/design-tokens.md`): `--blacksmith-disposition-friendly` / `-neutral` / `-hostile` / `-secret`. The module's own muted palette rather than Foundry's saturated canvas colours, because these render inside Blacksmith chrome rather than over the map. `node tools/check-design-tokens.mjs` passes.
...
- **Verify live**: start a combat with a friendly PC, a hostile NPC, and a neutral NPC. Each portrait carries a stripe in its own colour, and hovering names the disposition in words. Change an NPC's disposition on its token sheet - the stripe changes without reopening the bar. Kill a combatant - its stripe dims. Confirm a player sees the same stripes and words, including on an NPC's limited hover card.

### Changed - Round and Turn are badges, and statchips can be

- **`shape` on the statchip kind** (`scripts/api-menubar.js`, `templates/partials/menubar-secondary-item.hbs`, `styles/menubar-widgets.css`): `'pill'` (default, the label-then-value chip as before) or `'badge'`, which shows the value alone in a capsule and renders neither label nor icon. Documented in `documentation/api/api-menubar.md`. A shape rather than a new kind, because the markup, the tone and emphasis classes, and the patch path are all identical - only the label's presence and the box's radius differ.
- The capsule's minimum width equals its height, so a one-digit value is a circle and a longer one ("2 of 6") stretches sideways instead of ballooning the row.
- **The combat bar's Round and Turn readouts use it** (`manager-combatbar.js`). Between them the words "Round" and "Turn" cost more of the data row than the numbers they named, on a bar where the timer's width is pinned to the millimetre. They are also the one pair nobody needs told: their position never changes and they only count up.
- **Their tooltips now carry the meaning and the value together** - "Round 4", "Turn 2 of 6" - pushed with each update rather than fixed at registration, so a badge can never sit under a tooltip that does not mention its figure.
- **Fixed: `registerSecondaryBarItem` rejected a display item whose only content was `valueParts`.** The content guard tested `label`, `value`, and `image` and predates `valueParts`, so a chip stating its number in parts and naming itself only in its tooltip - exactly what the Turn badge is - could not register at all.
...
- **Verify live**: open the combat bar in a fight. Round is a circular badge holding the number, Turn a stadium holding "2 of 6", both with no labels. Hover each - the tooltip reads "Round N" and "Turn N of M" and tracks the numbers as turns advance. Advance past round 9 and confirm the round badge widens rather than clipping.
