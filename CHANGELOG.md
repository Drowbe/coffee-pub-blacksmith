...
- **Item Import UI** (`scripts/blacksmith.js`): Template dropdown expanded to seven types; copy uses composed prompts via `buildItemImportPrompt()`. Legacy `prompt-items-loot.txt`, `prompt-items-consumables.txt`, and `prompt-artificer-item.txt` are superseded (kept for reference until removed).
