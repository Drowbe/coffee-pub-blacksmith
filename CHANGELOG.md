...
- **Manage Pin Layers: all pin types visible regardless of taxonomy registration** (`scripts/window-pin-layers.js`): The Layers tab now scans actual scene pins after building taxonomy sections and adds a group for any `(moduleId, type)` combination not covered by a registered taxonomy. Unregistered types show global and custom tags from their pins — the same tags the user applied during pinning. Modules that only call `registerPinType()` (or skip registration entirely) are no longer invisible in the Layers window.
- **`registerPinTaxonomy` bulk format support** (`scripts/manager-pins.js`): The API now accepts the bulk form `registerPinTaxonomy(moduleId, { pinCategories: { type: { label, tags } } })` in addition to the per-type form `registerPinTaxonomy(moduleId, type, { label, tags })`. Prevents `[object Object]` taxonomy group labels caused by callers passing the full taxonomy object as the `type` argument.
