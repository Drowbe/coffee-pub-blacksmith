...
- **Whose turn it is moved to the border**, plus an inner glow, because the fill used to carry it. That reshuffle is the price of using the fill for disposition. Hover then had to stop using the border too - it made hover and the active combatant indistinguishable the moment a pointer crossed the wrong portrait - and lifts brightness instead.
