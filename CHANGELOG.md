...
- **Combat bar: readouts as registered items, with challenge rating (encounter bar merge, phase 4)** (`scripts/api-menubar.js`, `scripts/manager-combatbar.js`, `templates/partials/menubar-combat.hbs`, `styles/menubar-combatbar.css`): a bar type may now declare `hybridItems: true`, which makes `_prepareSecondaryBarData` fall through to the zone preparation instead of returning early for a custom template. Custom markup and registered items had been mutually exclusive, which left the combat bar unable to use the `info`, `progressbar`, and `balancebar` kinds even though those are exactly what its readouts need — a challenge rating chip, a health bar, a party-versus-monster balance, a timer. Because `menubar.hbs` invokes a custom partial with `secondaryBar.data` as its context, the prepared zones and banner settings are copied onto `data.data`; the combat template then hands its own context straight to the `menubar-secondary-default` partial and reuses the whole item rendering rather than restating it. The bar stays **one row**: the items render in the space the combatant-name endcap used to occupy, sharing the row with the portraits, grouped under a labelled banner the way the Broadcast and Cartographer bars are laid out. That endcap is gone because the highlighted portrait already says whose turn it is, and the round endcap on the left now carries the turn count with it, reading "Round 6" over "Turn 3 of 8" — its label-above-value emphasis was inverted for the new content, so it reads headline over detail. **Party CR, Monster CR, and Difficulty ship registered**, GM-gated since encounter difficulty is not player information, refreshed by the bar's own debounced `createToken` / `updateToken` / `deleteToken` hooks rather than `EncounterToolbar`'s — those are registered only when `enableJournalEncounterToolbarRealTimeUpdates` is on, and a readout on a permanently visible bar must not go stale because a setting named after journal toolbars was switched off. The labels are "Party" and "Monster" with the group banner carrying the heading, so the values drop the redundant "CR". **Verify live**: as GM out of combat, confirm the bar shows the challenge rating group at its right end with a banner heading, and that adding or removing tokens on the canvas updates the values within a moment; start a combat and confirm the portraits and the round endcap appear without the readouts moving or the bar changing height beyond the in-combat size; confirm the left endcap reads the round with the turn count beneath it and that no combatant-name endcap remains; and confirm as a player that the challenge rating group is absent entirely.
