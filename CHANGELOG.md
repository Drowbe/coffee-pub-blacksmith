...
- Expanded the GM Send Toast tool with optional recipient-side sound, a compact Journal Tools-style Font Awesome icon palette, mutually exclusive icon/avatar-image selection, clear buttons for avatar and background image paths, and client-side last-used preferences for recipients, style, size, duration, sound, icon, and images. Title and message intentionally start blank. `api.toast.show()` also accepts an optional `sound` path; reusable named toast templates remain a documented later feature.
...
- **Toast styles, true persistence, and a GM "Send Toast" party-bar tool** (`scripts/api-toast.js`, `styles/toast.css`, `scripts/manager-sockets.js`, `scripts/window-toast-send.js` new, `styles/window-toast-send.css` new + `@import` in `default.css`, `scripts/api-menubar.js`, both toast docs): three additions to the toast system, decisions locked with the author (semantic styles now with design-token mapping later; persistent toasts exempt from eviction; GM gets a small confirmation rather than an echo). **Appearance**: `show()` initially gained a whitelisted semantic `style` set, which the author replaced pre-release with a **`color` parameter** (the API takes parameters; presets live in the consumer): strict-hex validated, applied as the `--blacksmith-toast-accent` custom property, one value driving border/icon/title plus a `color-mix` tinted wash of the box background (fullscreen tints its scrim the same way) — the second sanitized inline exception to the class-only model, alongside `backgroundImage`. **Persistence**: `duration: 0` now means what it says — persistent toasts no longer count toward `MAX_STACK` and are never evicted (previously five subsequent transient toasts would silently evict a "persistent" announcement); only the ×, a `stackKey` replacement, or programmatic removal ends them. The cap still bounds transient toasts exactly as before. `getActive()` now also reports `persistent`/`style`/`size`. **GM tool**: a `Send Toast` button on the party menubar bar (GM-only, bullhorn icon) opens a small window — recipients (non-GM users; active ones pre-checked, offline disabled), title, message, style dropdown, duration (including "until closed"), and an image with FilePicker browse — and sends a `size: 'large'` toast to the selected players. Delivery rides a new **internal** targeted relay, `sendToastToUsers(config, userIds)`: targeting is receipt-side per the socket privacy rule (`_recipients` on the payload; the `showToast` handler renders only on listed clients; content is non-secret by contract). This is Blacksmith-private plumbing like `broadcastToast` — the public cross-client `send({recipients})` API remains gated on the socket rewrite. The sending GM sees a small "Toast sent" confirmation toast naming the recipients. **Second round (same day, author direction): background images and the toast/billboard split.** `show()` gains `backgroundImage` — a cover background behind the toast content (aspect-preserving `cover`, centered crop) with an automatic dark scrim for legibility, the one deliberate inline-style exception to the class-only model (path is `encodeURI`d so it cannot escape the `url("")` wrapper). The size model went through one iteration (viewport-*width* presets) before landing on the author's design: the display model is **binary** — no `size` = a toast (content-fit, stacks top-center); `size: 'small' | 'medium' | 'large' | 'fullscreen'` = a **billboard**: a viewport-proportional box in *both* dimensions (≈26×18 / 40×28 / 58×42 percent, tuned per preset rather than one shared percent so ultrawide monitors don't produce shapeless boxes; fullscreen is 100×100 with a dark scrim), centered on screen and rendered directly on `<body>` outside the stack, with clamp'd viewport-unit typography so content scales relationally with the box and overflow scrolling inside it (a fixed box cannot grow with its content). Billboards are singletons (a new one replaces the current, whatever its size), exempt from the stack cap like persistent toasts, and clicking anywhere dismisses (a real dismissal — `onDismiss` fires; author decisions 2026-07-19). Fullscreen's use case: "everyone rejoin the table" while the GM is heads-down with one player. The Send Toast window's Size dropdown reads Adapt to Content (default) / Small / Medium / Large / Fullscreen, and a second Background image field gets its own FilePicker browse. **Third round (author review of the layout): the window now speaks the shared Blacksmith window vocabulary instead of half-fighting it** — banner header zone enabled (bullhorn icon + title + subtitle, matching the importer windows), each section wrapped in the shared `blacksmith-window-section` card with its `blacksmith-window-section-header` iconed header (Recipients/Message/Appearance), `blacksmith-field`/`blacksmith-field-row`/`blacksmith-field-label` labeled fields from `window-form-controls.css`, labeled `Browse` buttons matching the importer's folder-open pattern, with `window-toast-send.css` reduced to pure layout (recipient grid, image rows). Also added per author: an **Entire Party** master checkbox (checks and locks every online player; send resolves recipients to all active players) and the message field is now a 3-row textarea. **Fourth round (author redesign of the appearance controls): templates and the background toggle.** The Style dropdown is gone; in its place a **Template** selector — built-in templates (`BUILTIN_TEMPLATES` in `window-toast-send.js`: Announcement/Information/Success/Warning/Danger, the former semantic styles reborn as color bundles; non-deletable) plus user templates saved by name (Save As with a DialogV2 prompt, Delete with confirm; user entries marked `*`) in the world-scoped `toastSendTemplates` setting. A template is an appearance bundle only (background mode + color/image, icon/avatar mode, size, duration, sound — never recipients or message); picking one stamps the form and editing diverges freely. **Background** became a color/image toggle: color mode shows a colorpicker whose value is sent as the `color` accent (border + tinted box background); image mode shows the background-image path + browse. The icon picker's first tile became **Custom image** — selecting it reveals the avatar-image field, selecting any icon hides and clears it, making image and icon mutually exclusive by construction. Along the way billboards were hardened against stale-CSS catastrophes: they render inside a fixed full-viewport layer whose positioning is inline and JS-owned, so a cached stylesheet can degrade a billboard to unstyled but can never drop it into Foundry's body layout and shove the UI around (observed live before the fix). Verify live (two clients): pick each built-in template → form stamps its color/icon/duration and the sent toast matches; save a custom template, reopen the window (it persists, `*`-marked), delete it (built-ins refuse); color mode → border + tinted background in the chosen color on the recipient; image mode → cover background with scrim; fullscreen (click anywhere closes; a second replaces the first); `duration: 0` toast survives six transients; targeted send renders only for the chosen player, GM gets the blue confirmation; Send Toast is GM-only in the party bar; offline users disabled in the picker.
...
- **NPC prompt now produces complete action bars and believable carried inventory** (`prompts/prompt-characters.txt`): every NPC receives the standard action references (plus Ready Spell for casters), while every gear-capable NPC receives a lean role-appropriate kit beyond signature weapons/armor. The rule strongly defaults to useful carried gear without padding random loot and explicitly exempts creatures that cannot plausibly carry equipment, such as ordinary beasts, oozes, incorporeal spirits, most constructs, and summons. Existing compendium content remains exact-name references; custom gear may use friendly or native inline definitions.
