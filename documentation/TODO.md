...
- **Chat-noise reduction — remaining candidates**, grouped by transport readiness (2026-07-17 survey of all `ChatMessage.create` sites):
  - *Ready now (existing receipt-side signal, no new sockets)*: **session timer set/start cards + warning/expired** (`api-menubar.js` timer card site — end time already syncs via the `updateTimer` socket, so warning/expiry can be evaluated and toasted per-client); **planning timer expired** (`timer-planning.js` :1145) and **combat timer warning/expired** (`timer-combat.js` :1161) — timer state reaches clients via combat flags/sockets already.
