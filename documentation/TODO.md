...
outside the phases and would otherwise vanish when that plan is deleted.

- **`timer-round.js` caches two DOM selectors that never match**: `_refreshRoundTimerDomCache` queries `.combat-endcap-left .combat-time-round` and `.combat-endcap-right .combat-time-total` (`timer-round.js:36-38`), but `combat-time-round` / `combat-time-total` exist only in `templates/timer-round.hbs` and have never been in the combat bar template. Both cache entries have always been empty, and both endcaps are now gone entirely. Either wire the round and combat timers into the bar's data row — `getCombatData` already computes `totalCombatDuration` and `currentRoundDuration` and the template ignores them — or delete `combatbarRound` / `combatbarTotal` from the cache, its clear, and its staleness check. Phase 7 of the plan covers the wiring; this entry survives if that phase is dropped. Verify live: with the combat bar open during an encounter, confirm the round timer widget still updates, and that whichever path is chosen leaves no selector in the cache that matches nothing.
...
- **`CombatTimer.state.isActive` is set by `resumeTimer` and nothing else**: `startTimer` never assigns it (`timer-combat.js:770` is the only write), so the flag reads as false for a turn timer that started normally and only becomes true after a pause and resume. It is not merely misnamed — `startTimer` *announces* `isActive: true` in the `combatTimerStateChange` payload it emits (`:697`, `:705`), so the hook and the object contradict each other, and any consumer trusting the hook disagrees with any consumer trusting the state. One internal consumer is already weakened by it, though not broken: the DOM cache self-heal in `updateUI` (`:890`) reads `this._combatTimerDomCacheAllEmpty() && this.state.isActive && ...`, so that arm cannot fire for a normally-started timer. **Severity is low** — it is the second arm of an `||`, the first (`_isCombatTimerDomCacheStale()`, cached nodes disconnected) does not depend on the flag, and `_onRenderCombatTracker` refreshes the cache outright on every tracker render, which Foundry does on essentially every combat update. The dead arm therefore only covers the window between a `cleanupTimer()` cache clear and the next render, and that window closes by itself. This is a latent inconsistency to tidy, not a live fault. The combat bar deliberately avoids the flag and uses `CombatTimer.shouldDisplay()` instead, but the next consumer will reach for the obvious-looking name and get a wrong answer with no symptom to trace. Fix by assigning it in `startTimer` to match the hook payload it already sends, then re-check the DOM-cache guard once it means what it says. Verify live: start a combat with the turn timer enabled and confirm `CombatTimer.state.isActive` is true without pausing; confirm the tracker's timer and the combat bar's both still update across a tracker re-render and a pause/resume; and confirm the planning-to-turn handoff is unaffected.
- **Tint the combat bar**: the data row currently sits on `rgba(0, 0, 0, 0.25)` over the bar's own background, chosen only to separate the two rows. Decide the bar's colour treatment as a whole — the data row's tint, the combat row's background, and whether either should shift with encounter state (difficulty, or party-versus-monster balance once that lands) rather than staying constant. Verify live over light and dark scenes, since the bar is translucent and reads against whatever is beneath it.
- **Decide whether the canvas-clearing actions hide during combat**: Remove Party / Monsters / NPCs from Canvas show in the Tokens menu at all times. The plan's target state has them hidden while an encounter runs, on the grounds that they are out-of-combat tools. This was deferred while the bar could not exist outside combat, which made hiding them equivalent to removing them; that is no longer true, so it is now purely a presentation call. Verify live: confirm the three rows are absent during combat and present outside it, and that Reveal Hidden — which *is* a mid-combat tool — stays in both.
- **Confirm which module owns Quick Encounter**: the combat bar and the encounter bar both gate the row on `!!api.hasQuickEncounterTool?.()` (`ui-journal-encounter.js:1573`), so the capability comes from elsewhere. Identify the owner and confirm whether it adds monsters to the *current* encounter or only creates a new one — the answer decides whether the row belongs beside Create Combat or in its own group. Verify live: with the providing module disabled, the row is absent from both bars and neither logs an error.
