...
## Compendiums: a filtered query, and the index projection that has to widen for it (opened 2026-08-23)

Contract shapes and the design decisions are in `plans/plan-compendium-query.md`. The decision to take the
work at all, and Merchant's reasoning for asking, are in `TODO-GLOBAL.md`. This section is the work.

**Do these in order.** Each is verifiable alone, and steps 1 and 2 must leave `search()` and `resolve()`
behaving exactly as they do now -- the existing 57 assertions in `testing/suites/suite-compendiums.js` are
what says so, and they are the reason the risky parts go first while there is nothing new to blame.

- [ ] **Widen the index projection, behind a flag nothing sets yet.** `_getPackIndex`
      (`manager-compendiums.js:661`) currently maps each entry to `{name, type, img, uuid}` and discards the
      rest, so a widened Foundry index changes nothing until this widens with it. It gains an `extended`
      flag -- not a field list, because `getIndex({fields})` re-fetches the whole index for every distinct
      superset (`client/documents/collections/compendium-collection.mjs:332`) and free-form field sets
      therefore cost one full re-fetch each, per pack, for the session. The declared set is
      `system.rarity`, `system.price.value`, `system.price.denomination`. Two cache states only: a base
      request accepts either, an extended request against a base entry discards and re-fetches, and an
      extended entry is never downgraded.
      **How to verify:** the existing suite passes unchanged, because nothing yet requests the flag.

- [ ] **Extract the scan out of `searchDetailed` into a shared engine.** The plan builder, source-order
      union, uuid dedup, world-vs-pack split and source labelling are all reusable as they stand; `query()`
      must not fork them. The engine takes two explicit mode flags where the callers genuinely differ:
      whether a needle is required, and whether `limit` stops the scan or caps the output after a complete
      one. `searchDetailed` becomes its needle-mode caller and keeps stop-scan.
      **How to verify:** pure refactor -- the same 57 assertions pass with no edits to the suite. If any
      assertion needs changing, behaviour moved and the step is wrong.

- [ ] **Economics helpers, exposed on the API.** `normalizeRarity(token)` and `toGp(price)`. Three dnd5e
      facts a consumer cannot verify and would each get wrong once: rarity is `""` for mundane gear, not
      `"common"` (`dnd5e.mjs:14077`), so the vocabulary needs an explicit `mundane` token; price carries a
      denomination with gp as the pivot, so 50 sp is 5 gp and a raw compare on `system.price.value` is
      wrong; and `price.value` has `initial: 0`, so unpriced and free are the same stored value.
      **How to verify:** assertions for `very rare` / `veryrare` / `Very Rare` all normalizing to
      `veryRare`, `mundane` round-tripping to `""`, an unknown denomination yielding `null` rather than
      being treated as gp, and a live non-gp-priced entry converting correctly.

- [ ] **`query()` and `queryDetailed()`,** then the API delegates and JSDoc in `api-compendiums.js`. Rows
      carry the `search()` shape plus `rarity`, `priceGp`, `price` and `matchType: null`; `search()` carries
      the same three keys, `null` when the index was not extended, so a key never appears and disappears
      depending on which call produced the row. Filters live on the engine rather than on `query()` alone,
      because the window needs a rarity facet that works while text is being typed. An entry lacking a
      filtered field fails the filter rather than passing unfiltered.
      **How to verify:** below.

- [ ] **Harness assertions in `suite-compendiums.js`,** deriving fixtures from the live world as the suite
      already does. Two of them are the ones a future reader would otherwise "fix" back, so they get the
      same treatment the three search-vs-resolve differences got: a `limit: 1` query still reports every
      configured source in `scannedSources`, where the same limit on `search()` reports a truncated scan;
      and `rarity: ['common']` returns no blank-rarity entry while `rarity: ['mundane']` returns nothing but
      blank-rarity entries. Plus: `priceGp` excluding unpriced by default, the projection surviving a base
      search issued after an extended query, and the extended keys present on every row.

- [ ] **Consumer zero: facets in `window-compendium-search.js`.** Not a token adoption -- it changes what
      the window can do. The palette refuses to run below `MIN_QUERY_LENGTH`, so there is currently no way
      to browse; with a rarity or price facet set, an empty query box becomes a valid browse and the window
      switches to `queryDetailed`. Facets follow the rule `_availableSubtypes` already establishes: offered
      only for Item-backed types, hidden in All mode. Rows gain rarity and price badges, the stored
      preference set gains both, and `window-compendium-search.css` gains the badge styling.
      **How to verify:** in a live world, open the palette with an empty box and a rarity facet set and
      confirm rows appear; confirm the facet is absent in All mode and for a non-Item type; confirm typing
      text alongside a facet narrows rather than replacing; confirm the status line still reports source
      count and timing, and does not claim truncation for a complete scan; confirm drag-to-sheet still
      works from a browse row, since those rows never carried a `matchType`.

- [ ] **Write `architecture-compendiums.md`.** It does not exist. `manager-compendiums.js` is 1159 lines
      holding a resolver, a search engine, an index cache and now a query engine, plus a legacy surface, and
      everything a reader needs about it has to be recovered from source -- which is the exact failure the
      architecture docs exist to prevent. Cover the two-state index cache and why it only widens, the two
      scan modes and why `limit` means different things, the deliberate resolve/search/query ordering
      differences, and the dnd5e economics traps.

- [ ] **Add the query section to `api-compendiums.md`,** including the performance note: the first query
      needing economics costs one extra index fetch per configured pack, once per session.

- [ ] **Delete `plans/plan-compendium-query.md`** once the above are distributed, and remove the
      `TODO-GLOBAL.md` entry once Merchant has adopted it.
