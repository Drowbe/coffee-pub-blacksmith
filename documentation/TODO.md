...
- **Status: Blood Damage + Blood Hit + Remove All Blood IMPLEMENTED (2026-07-22) — pending live verification.** Blood Damage: growing central pool + small splats on the ground under the token (under, not over the art) via `PrimarySpriteMesh` in `canvas.primary` at sort layer 650; tiers from the shared `scripts/utility-health.js` helper. Blood Hit: transient burst above the token on damage taken, scaled to the hit. Remove All Blood / Restore All Blood: paired GM toolbar buttons (`clear-blood` / `restore-blood`) relayed to all clients via hidden world settings (`tokenBloodClearRequest` / `tokenBloodRestoreRequest`); cleared tokens re-bloom on new damage, and Restore lifts all suppression and redraws from current HP. Settings `tokenBloodEnabled` (Blood Damage) / `tokenBloodHitEnabled` (Blood Hit) / `tokenBloodVisibility` in the Health Indicators section. Full entries with per-feature verification steps in `CHANGELOG.md` [13.10.4]; plan at `documentation/plans/plan-token-blood.md` (dismantle after verification). **Core verify matrix**: damage a linked PC and an unlinked NPC — any damage at all shows the small 'hurt' pool, then past each tier boundary (75/50/25/0%) → pool grows on all clients and a burst fires per hit; heal → pool recedes (gone at >=75%), no burst; kill → widest pool; drag → follows; hide token → players lose it, GM keeps it; `gmOnly` visibility → players never see either; Remove All Blood → clears on all clients, returns on next damage; disable each setting live → its half stops; perf monitor shows no idle cost.
- **Remaining after verification**: optional authored splatter art replacing the procedural texture; rewire the combat bar and party bar onto `utility-health.js` so the HP math has one home.
- **Original issue**: no at-a-glance visual of token health on the canvas. Idea: a blood-splatter overlay per token whose splatter tier follows % remaining HP (thresholds like bloodied/critical), optionally with a brief animation on damage taken.
