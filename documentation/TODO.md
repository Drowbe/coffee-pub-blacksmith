...
- **Shipped as sketched**: GM-only pointer drag on `.combat-portrait-container` (8px threshold before a press commits, so click/double-click/hover/context-menu are untouched); drop writes the neighbor-midpoint initiative via `combat.setInitiative` (edges: top + 1 / bottom - 1; null initiative counts as 0; ties join at the shared value; same-slot drops write nothing); `updateCombatBar()` defers mid-drag rebuilds and the post-drag click is consumed so it cannot pan. Escape cancels. Visuals are tracker-style: the portrait dims in place, a ghost of it rides the pointer, and dropzones injected between portraits spread the row with the nearest lighting up (`styles/menubar-combatbar.css`); every client gets a broadcast "«name» moved to position X of Y" toast with the new initiative.
- **How to verify**: as GM drag a portrait between two others — ghost follows the pointer, row spreads with dropzones, nearest zone lights, bar and native tracker reorder identically on all clients, initiative reads as the midpoint, and the position toast shows on every client; drag past either end → first/last; Escape mid-drag → no change and everything cleans up (no ghost or zones left behind); click and double-click still pan / set current; same-slot drop (either zone beside the dragged portrait) fires no update and no toast; players cannot drag.
