...
- **Status: v1 IMPLEMENTED (2026-07-22) — pending live verification.** Ground splatter under the token (author direction: under, not over the art) via `PrimarySpriteMesh` in `canvas.primary` at sort layer 650; tiers from the shared `scripts/utility-health.js` helper; settings `tokenBloodEnabled` / `tokenBloodVisibility` in the new Health Indicators section. Full entry in `CHANGELOG.md` [13.10.4]; plan at `documentation/plans/plan-token-blood.md` (dismantle after verification). **Verify live**: damage a linked PC and an unlinked NPC past each tier boundary (75/50/25/0%) → splatter appears and intensifies under the token on all clients; heal → recedes, gone at >=75%; kill → dead pool; drag → splatter follows; hide token → players lose it, GM keeps it; `tokenBloodVisibility: gmOnly` → players never see it; disable `tokenBloodEnabled` or `generalIndicatorsEnabled` → all splatter removed live; perf monitor shows no idle cost.
- **Remaining after v1 verifies**: damage-taken flash animation (phase 2, reuses the manager's PIXI animation pattern); optional authored splatter art replacing the procedural texture (phase 3); rewire the combat bar and party bar onto `utility-health.js` so the HP math has one home.
