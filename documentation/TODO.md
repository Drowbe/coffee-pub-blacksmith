...
- **Design decision (author, 2026-07-18): rectangle stays — it is the image-only shape.** With a FontAwesome icon it is forced square (identical to Square); its unique behavior is **image URL + Rectangle**: height derives from the image's natural aspect ratio against the chosen width, with the rounded-corner background/border (`window-pin-configuration.js:114-121`). `none` gives the same free aspect without the background. The author initially suspected rectangle was a dead leftover from removing manual width+height — it isn't, but only the image path distinguishes it.
- **Save round-trip VERIFIED live (2026-07-18).** Verification surfaced a second bug the dead save path had hidden: the container's CSS `border-radius: 15%` skews into **elliptical** corners on non-square pins (percentage radii resolve per-axis) while the inner image was already clipped to a px radius — fixed in `pins-renderer.js` by pinning the container to the same short-side px radius in the zoom-aware update; `pins.css` variable comment updated to state the constraint.
- **Still to verify (live)**: rectangle + image pin corners now render as circular 90° arcs (outer border and image clip agree) at multiple zoom levels; square pins look unchanged; `api.pins.update(id, { shape: 'rectangle' })` still round-trips. (An FA-icon rectangle staying square is correct, not a failure.)
- **Doc follow-up**: `architecture-pins.md`'s shape list intentionally mirrored the buggy whitelist — correct it to include rectangle, described as the image-only free-aspect shape (documentation agent).
