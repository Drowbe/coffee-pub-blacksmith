...
- **`CombatTimer.state.isActive` is set by `resumeTimer` and nothing else**: `startTimer` never assigns it (`timer-combat.js:770` is the only write), so the flag reads as false for a turn timer that started normally and only becomes true after a pause and resume. It is not merely misnamed — `startTimer` *announces* `isActive: true` in the `combatTimerStateChange` payload it emits (`:697`, `:705`), so the hook and the object contradict each other, and any consumer trusting the hook disagrees with any consumer trusting the state. There is already one internal casualty: the DOM cache self-heal in `updateUI` (`:890`) is guarded on `this._combatTimerDomCacheAllEmpty() && this.state.isActive && ...`, so the "cache went empty while the timer is running, refresh it" recovery cannot fire for a normally-started timer, only for a resumed one. The combat bar deliberately avoids the flag and uses `CombatTimer.shouldDisplay()` instead, but the next consumer will reach for the obvious-looking name. Fix by assigning it in `startTimer` to match the hook payload it already sends, and audit the DOM-cache guard once it means what it says. Verify live: start a combat with the turn timer enabled and confirm `CombatTimer.state.isActive` is true without pausing; re-render the combat tracker and confirm the timer bar keeps updating rather than freezing until a pause/resume; and confirm the planning-to-turn handoff is unaffected.
