...
- **Tune the data row height and item font ratio**: the data row is a constant `CombatBarManager.DATA_ROW_HEIGHT` (38) with the item font and icon at `calc(height * 0.36)` clamped 10-15px (`styles/menubar-combatbar.css`). Both were starting points chosen before the row was seen live. Group banners are off, which freed roughly 12px of vertical, so the row can likely come down. Verify live at several combat-row sizes: the readouts must not change size when the combat row does.
