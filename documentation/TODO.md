...
- **Status**: CORE EXTENSION IMPLEMENTED — synchronous and async document resolution, bulk reads, write capability/typed failures, future-field-preserving envelopes, JournalPage breadcrumb hook context, locked-pack read-only handling, and the reusable `createField()` / `renderField()` controller are available. The legacy dnd5e Item/Container card remains supported.
