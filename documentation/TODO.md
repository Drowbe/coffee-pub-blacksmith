...
- **Need**: a transient on-screen "toast" that pops up over the play area — distinct from the menubar notification strip. Suite-wide primitive: Bibliosoph already built its own (`coffee-pub-bibliosoph/scripts/manager-conversations.js` `_showSplash` :1252 — avatar image, title + subtitle, whole-card click opens the conversation, 8s fade, one-at-a-time replace-in-place) and that pattern drift is exactly what the design-system extraction fights. The rendering primitive moves into Blacksmith; consumer *policy* (which events toast, mention rules, auto-open fallbacks) stays in the consumer.
- **Phase 1 — local primitive (NOT gated on anything)**: `api.toast.show({ title, subtitle, icon, image, duration, onClick, onDismiss, stackKey })`. Renders on *this* client only; cross-client events already reach clients via each module's own transport (Bibliosoph's message flow, Blacksmith's own setting sync), so the receipt side just calls `show()`. Single body-click action only — click anywhere on the toast runs `onClick`, then removes it. `onDismiss` follows the **same dismiss-semantics table as menubar notifications** (`api-menubar.md`): fires on timeout and user-close only, never after `onClick`. `stackKey`: toasts stack (capped) by default; same `stackKey` replaces in place (Bibliosoph's splash behavior, Squire's dedup pattern). `image` slot in addition to FA `icon` — Bibliosoph shows the sender's avatar. Not Foundry's `ui.notifications` (core-styled top-center text queue; not actionable, not themeable, no images) — say so in the docs.
- **Phase 2 — actions beyond the body click**: optional `actions: [{ label, onClick }]` button row for multi-choice toasts ("roll for crit" / "read message" / "acknowledge"). Phase 1's single body-click API must not change shape when this lands — actions are additive.
...
- **Dogfood test (Phase 1) — leader change**: `setNewLeader` (`api-menubar.js` :4885) posts two chat cards today — a public "new leader" card and a whisper to the new leader. Every client already syncs the `partyLeader` world setting, so each client can toast receipt-side with no sockets: "You are now the leader" on the leader's client, "«Actor» is now the leader" elsewhere. **Long-term goal: replace that chat noise with toasts — but NOT yet**; for now the toast runs alongside the chat cards.
- **Future-proofing ideas (captured, not committed)**: per-toast sound; priority/queue ordering when stacked; themes via the design-system tokens; more chat-noise migrations once the leader dogfood proves out (vote results, timer expiry); Phase 3 ack-back ("player clicked acknowledge" reported to the GM).
- **Docs when it ships**: `documentation/architecture/architecture-toast.md` (how it renders, stacking model, why local-first) + `documentation/api/api-toast.md` (surface + the shared dismiss table, referenced from `api-menubar.md`, not duplicated). New files: `scripts/api-toast.js`, `styles/toast.css` — **must be `@import`ed in `styles/default.css`** or it's silently unstyled. Feature-sized: needs a plan file per the workflow before implementation.
- **How to verify (Phase 1)**: console `api.toast.show({...})` → toast appears, auto-dismisses after duration with fade; body click runs the handler and removes it without firing `onDismiss`; timeout/close fires `onDismiss`; two toasts with the same `stackKey` → the second replaces the first; different keys stack. Leader dogfood: change leader → leader's client and other clients show differentiated toasts; chat cards still post.
- **Priority**: Medium (feature). Phases 1–2 are unblocked; only Phase 3 is gated on the socket system.
