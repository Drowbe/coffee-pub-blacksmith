...
- **Two stylesheets are loaded by nothing.** `styles/journal-toolbars.css` (52 lines) and `styles/widget-tags.css` (154 lines) appear in neither `styles/default.css`'s import list nor `module.json`'s `styles` array, so none of their rules ever apply. Decide per file: wire it into `default.css` if the styling is wanted, or delete it. Check whether the corresponding features render unstyled today before assuming deletion is safe — `widget-tags.css` in particular pairs with `scripts/widget-tags.js`, which is live.
