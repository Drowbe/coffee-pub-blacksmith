...
- **Chat-noise reduction (later, deliberate step)**: once the leader dogfood proves out, replace the two leader chat cards in `setNewLeader` (`api-menubar.js`) with the toast; candidates after that: vote results, timer expiry. NOT yet — toast currently runs alongside the cards.
