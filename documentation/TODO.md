...
### ✅ FIXED (2026-07-18) — XP window defaulted non-combatants to included after combat — **pending live verification**
- **Scope (author, 2026-07-18)**: the whole party stays listed; only the *inclusion toggle default* changes — non-participants open unchecked (dimmed), and the GM can still toggle anyone in.
- **Root cause**: two independent halves. (1) `calculateXpData` loads `loadPartyMembers()` (whole party, all `included: true`) and never consulted `combat.combatants` — `getCombatPlayers()` exists but has **zero call sites** (the two-entry-point mismatch from the `architecture-xp.md` audit). (2) The template hardcoded the toggle as `class="inclusion-toggle active"`, ignoring `player.included` entirely — so even correct data would have rendered checked.
- **Fix applied**: `calculateXpData` now marks each party member `included` by membership in the combat's player-owned combatants, and `partySize` (which drives the party multiplier and per-player split) counts only included players, so the window opens with math matching its toggles. `window-xp.hbs` renders `active`/`dimmed` and the per-row totals from `included`. `autoDistributeXp` no longer force-resets everyone to included — it preserves participation defaults and gives excluded players `finalXp: 0` (it previously mimicked "everyone in"). Manual `openXpDistributionWindow` (no combat) is unchanged — whole party, all included.
- **How to verify (live)**: run a combat with only some party members as combatants → on end, the XP window lists the whole party but only participants have the red (active) toggle; Distribution Size and Per Player reflect participants only; toggling a non-participant in recalculates; Distribute applies XP only to active-toggle members. With `autoDistributeXp` enabled, only participants receive XP and the chat card shows them alone. Manual menubar open (no combat) still defaults everyone included.
...
#### Dead `settingChange` hook registrations — the hook does not exist in Foundry
- **Issue**: verified against Foundry v13.351 core source (2026-07-17): **no core hook named `settingChange` exists** and nothing in Blacksmith fires one, so every registration against it has never fired once. Core fires `clientSettingChanged` (client-scoped settings, changing client only) and the `updateSetting`/`createSetting` document hooks (world-scoped settings, all clients). Discovered when the toast leader dogfood didn't fire; the menubar leader display had always synced through the socketlib `updateLeader` broadcast instead, masking the dead hook. See the ⚠️ block in `architecture-blacksmith.md` §9B.2.
- **Sites (all dead)**: `api-menubar.js` (session-timer settings hook; one more at the old :666 site), `blacksmith.js` (settings-cache invalidation), `manager-combatbar.js` (×2), `manager-journal-tools.js`, `manager-token-indicators.js`, `manager-toolbar.js`, `sidebar-combat.js` (raw `Hooks.on`, not HookManager), `ui-journal-encounter.js`, `ui-sidebar-style.js`, `ui-sidebar-pin.js`. The menubar **partyLeader** site is already fixed (rewired to `updateSetting`+`createSetting` in `_registerLeaderChangeHook`).
- **Need**: audit each site and rewire per the setting's scope — world → `updateSetting`+`createSetting` (`setting.key` is `"module.key"`; `setting.value` is **already cast to the registered type** by the client Setting document — only `_source.value` is the raw JSON string, so do not `JSON.parse` blindly); client (e.g. the sidebar pin/style user prefs) → `clientSettingChanged`. **Do NOT blanket-remap `settingChange` in HookManager**: (a) the right target hook differs by setting scope, and (b) these callbacks have never executed — resurrecting all of them at once may surface latent double-sync or GM-assumption bugs (the session-timer callback, for instance, does socket sends). Activate one site at a time, each with its own verification.
- **How to verify**: per site — trigger the setting change and confirm the callback observably fires (console log / behavior) on the clients its feature needs; specifically re-test features that "worked" via compensating paths for double-handling.
- **Priority**: High — settings-cache invalidation, combat bar, token indicators, and sidebar reactions to setting changes are all silently inert.
