...
| Windows | `registerWindow`, `unregisterWindow`, `openWindow`, `getRegisteredWindows`, `isWindowRegistered`, `getWindowBaseV2`, `BlacksmithWindowBaseV2`, `getToolWindowBaseV2`, `BlacksmithToolWindowBaseV2`, `windowStyles`, `toolTitlebars` | `api-window.md` |
...
|---|---|---|
| `deployParty` | `({ pattern?, prompt?, includeOwnedNpcs? }) => Promise<Token[]>` | Places the party's player characters on the current scene, skipping any that already have a token there. With no arguments it asks which formation to use (grid, circle, line, scatter, sequential) and whether to include player-owned NPCs, remembering both answers as the defaults. The NPC checkbox appears only when there are owned NPCs not already placed. Pass `pattern` to skip the picker, `includeOwnedNpcs` to decide the roster explicitly, or `prompt: false` to use the saved defaults silently -- a scripted caller is never blocked on a dialog. Dismissing the picker returns an empty array. GM only: a non-GM gets a logged warning and an empty array. |
