...
8. **Use `group` and `tags[]` for user-facing layering**: Keep `type` coarse/technical, and use `group` plus `tags[]` for filterable user-facing classification.
...
interface PinData {
  ...
  group?: string; // User-facing grouping bucket (e.g. 'journal', 'locations', 'quests'). Normalized to lowercase kebab-case for storage/filtering.
  tags?: string[]; // User-facing classification tags. Supports registered and freeform values; normalized to lowercase kebab-case.
