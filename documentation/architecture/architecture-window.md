...
- **`BlacksmithToolWindowBaseV2`** (`scripts/window-tool-base.js`) extends the same lifecycle with compact defaults, a dedicated tool template, shared Light parchment and Dark standard-window visual shells, optional inline native-title actions, tool-body scroll preservation, and position persistence. Frame styling belongs to the API; consumers style only their body content unless intentionally overriding the exposed Tool custom properties. `toolTitlebar` selects the full title bar or a micro native drag rail; `toolTheme` selects the Light or Dark shell. Both choices use mutable runtime state rather than Foundry's frozen options and persist independently per tool. Both title bars launch Blacksmith's shared, document-level `UIContextMenu` instead of Foundry's frame-owned controls dropdown, preventing menus from being clipped by compact tool geometry. The menu switches title-bar mode and theme unless the consumer locks or disables their preference storage.
