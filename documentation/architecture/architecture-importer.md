...
Compendium Mapping may operate manually or in authoritative automatic mode. Automatic mode inspects pack indexes, filters each mapping to compatible document types/subtypes, and orders sources by official supplement/update, core PHB/DMG/MM, third-party/imported/homebrew, then bundled SRD. Blacksmith generates one inclusion checkbox per installed Foundry package/source, not per individual pack. That source allowlist is reused by every automatic mapping and filters both the choices and effective sources of every manual mapping after the settings are saved and Foundry reloads. Disabled-source values remain stored but inactive. Manual mapping values remain stored and are restored when automatic mode is disabled.
