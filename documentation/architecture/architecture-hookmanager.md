...
> `context` on the record is load-bearing and easy to drop. It is *also* tracked in the `contexts` map, but the stats readers (`BlacksmithAPIHookStats`, hook details) read it **off the record**. It was missing until 13.9.x, so every hook reported as context `default` — making the context breakdown, the one thing those tools exist for, useless.
...
**Two behaviours that fall out of this and surprise people:**
...
> **Dead code:** `_throttle()` (`:466`) and `_debounce()` (`:480`) are defined and **never called** — `registerHook` inlines both. Don't mistake them for the live path.
...
> **There is no module-unload auto-dispose, and there cannot be one.** `closeGame` and `unloadModule` are **both dead names** — neither appears anywhere in Foundry v13, and nothing (Foundry, Blacksmith, or any installed module) ever fires them. Foundry has no runtime module-unload event: enabling or disabling a module reloads the world, which tears everything down anyway. Do not add a listener for either and consider cleanup handled. Tracked in `documentation/TODO.md`.
...
> **Known stubs — do not trust these outputs:**
> - `getHooksByCategory(category)` (`:454`) **ignores its argument** and returns every hook. There is no category system; the source says so.
> - `showHookDetails()` prints a hardcoded `Categories: [general]` (`:419`) for the same reason.
> - `showHookDetails()` walks priorities 1..5 only, so out-of-range hooks vanish from it.

---

## Document history

**2026-07-17 — rewritten from 1,411 lines to this.** The previous version was ~90% cuttable and actively misleading. It contained:

- **A 398-line verbatim copy of the `HookManager` class**, which had drifted into being wrong. It still showed the `callbackId.split('_')[0]` bug removed in this same cycle, omitted `context` and `teardown` from the callback record, presented `_throttle`/`_debounce` as the live path, and lacked the `pre*` cancel entirely. **Every one of those defects existed *because* the copy existed** — hence the pointers above.
- **An invented rule**: "⚠️ Parameter order is strict and must be exact!", taught in multiple places. `registerHook` takes a destructured object literal; order is meaningless.
- **A 155-line section** asserting "Only one callback per hook name" and "Module B will OVERWRITE Module A's hook!", then proposing multi-callback priority dispatch as *future* work — a feature that has shipped since `:139-142`. A contributor reading it would conclude the manager was broken and rebuild what already works.
- **~300 lines of one-time migration runbook** (rollback procedure, backup strategy, testing checklist) — not architecture.
- **~180 lines duplicating `api-hookmanager.md`**, worse than the original.
- Phantom examples: `closeGame`, `userLogin`, `searchInput`, plus two sibling-module internals (`PanelManager`, `StatsManager`) that appear **zero** times in Blacksmith.

None of the real internals documented above appeared anywhere in it. All of the deleted content remains in `git log` if it is ever wanted.
