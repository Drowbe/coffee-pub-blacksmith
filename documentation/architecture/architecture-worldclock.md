...
- **The gradient stops match `SUNRISE` and `SUNSET`.** The sky gradient in `worldclock.css` is hand-drawn
  to match the constants in the manager. Move one without the other and the marker crosses a painted dawn
  at the wrong time, which reads as "the colours look off" rather than as a bug.
