...
4. **Menubar rerender path is still broad**
   ...
   - **Evidence (mitigated 2026-03-28)**: `renderMenubar` compares `_computeMenubarStructureFingerprint(templateData)` to `_menubarStructureFingerprint`; if equal and primary container exists, **`_applyMenubarLightweightRefresh`** updates leader/movement/timer labels only (no `remove`/`insertAdjacentHTML`). **`updateLeaderDisplay`** calls full `renderMenubar(true)` only when **`_lastMenubarIsLeader`** changes (leader-only tools strip). Session timer still uses **`updateTimerDisplay`** (1s) without full menubar rebuild.
