...
Micro mode is a thin native drag rail with a single-dot launcher on hover/focus. The same document-level Blacksmith context menu opens on right-click and includes Squire's registered tool actions, Minimize/Restore, Reset Position, and Close. Because it is not a child of the tool frame, it remains usable at the tool's minimum dimensions. Squire should not add another close button or context menu.
